...
| `shielded/` | masked GPU offload: inference on an *untrusted* GPU host, where activations and KV cache never leave the trusted domain in plaintext. The mechanism behind Enclave Shield's GPU rule (a GPU on a non-TEE host is exposed only this way) and the large-model performance research on it, which is a separate line of work from the isolation layer. Engine code exists (CUDA and Vulkan workers here, the TEE-side ggml backend under `wasm/ggml-shielded/`) and runs on one self-hosted confidential server; not on the hosted fleet ([docs](docs/shielded-inference.md)) |
