...
**Compute that cannot see your data.** Enclave is a verifiable compute platform: publish a WebAssembly app once to an on-chain catalog, fund a deployment straight from your wallet, and it runs on an independent host whose protection level is stated on its listing. On confidential hardware (AMD SEV-SNP CPUs + NVIDIA confidential computing on today's hosted fleet, and self-hosted confidential servers) neither the operator nor the host can see into it: TLS terminates inside the enclave and the whole chain of trust, from the CPU's attestation quote down to the exact commit of this repo that built the running image, can be verified from a browser before you send the service a byte. Other hosts (today: one Windows PC in a development tier) are labeled with what their owner can still see. Billing is per-second on Base. The long version, with every protection level, the research status and what still runs on company servers, is https://enclave.host/architecture (source: `site/architecture.html`); the per-app hardware isolation research lives under `isolation/`.
...
| `isolation/` | per-app hardware isolation research (one execution model across ordinary and TEE hardware): the backend-neutral app-domain contract (`contract/`), and milestones M1 to M4 with their evidence scripts; lab results on one host, nothing in the product starts a domain yet |
