...
| `shielded/` | masked-offload tier (experimental): inference on an *untrusted* GPU host, where activations and KV cache never leave the CVM in plaintext. Engine code exists (CUDA and Vulkan workers here, the TEE-side ggml backend under `wasm/ggml-shielded/`) and runs on one self-hosted confidential server; not on the hosted fleet ([docs](docs/shielded-inference.md)) |
