#!/usr/bin/env bash
# rs-5 live acceptance (rs-4-accept.sh's shape; the release is ON for the 3 canaries since 4b). apply:
#   1. the api-relay restarted by rs-5 (a new invocation, 0 restarts after); the predictor's KAT PASS in THAT invocation;
#   2. accept.sh with ADMIT=52156652 AND with ADMIT=79c5ecf2: every canary's own measurement predicted, that release
#      predicted and admitted for it, 404, 422, and exactly one FAIL, 'release-ticket answered 403' (= the release ON);
#   3. for every canary: installed = {5c3561f9, 6f14ce75, a4f22748, 79c5ecf2, 52156652}, admitted = {79c5ecf2, 52156652},
#      and the 52156652 measurement is the one derived independently (63's expected-measurement.sh --pin, = mine) while
#      the 79c5ecf2 one is unchanged (what the canaries run now; 4e MATCH);
#   4. release ON for exactly the 3 canaries (listed:true x3, a69dcbba false); MemoryPeak under 1536M; /enclaves 200;
#      the canaries 200 over valid TLS with the key each guest printed at boot; us-west listed at 5.78.85.108; metal-iso0
#      serving and eligible.
# rollback: the same with 79c5ecf2 alone admitted and 52156652 not installed (item 3's 52156652 values not asked).
...
MODE=${1:?usage: rs-5-accept.sh apply|rollback}; H=$(cd "$(dirname "$0")" && pwd)
...
case $MODE in apply) ADM="$R7 $RN"; SET="$R5 $R6 $RA $R7 $RN" ;; rollback) ADM="$R7"; SET="$R5 $R6 $RA $R7" ;; *) echo "apply|rollback"; exit 2 ;; esac
...
ok=1; bad() { say "RS-5 ACCEPT FAIL: $*"; ok=0; }
prop() { $NAN "systemctl show enclave-api-relay -p $1 --value"; }
as=$(prop ActiveState); nr=$(prop NRestarts); inv=$(prop InvocationID); inv0=$(cat "$RS/rs5-$MODE-inv0.txt" 2>/dev/null || true)
say "api-relay: $as, NRestarts $nr, invocation ${inv:0:12} (before rs-5: ${inv0:0:12})"
[ "$as" = active ] && [ "$nr" = 0 ] || bad "the api-relay is not active, or restarted"
[ -n "$inv0" ] && [ "$inv" != "$inv0" ] || bad "the api-relay was not restarted by rs-5"
end=$(( $(date +%s) + 900 )); kat=""
...
for a in $ADM; do
  out=$(ADMIT=$a bash "$H/../accept.sh" 2>&1); echo "$out" > "$RS/rs5-$MODE-accept-${a:0:8}.txt"; echo "$out" | grep -vE '^\.\. '
...
ok = got == want and adm == adm_want and m.get(sys.argv[4]) == sys.argv[5] and (sys.argv[6] == "-" or m.get(sys.argv[7]) == sys.argv[6])
sys.exit(0 if ok else 1)' "$eg" "$SET" "$ADM" "$R7" "${M7[$c10]}" "$([ $MODE = apply ] && echo "${MN[$c10]}" || echo -)" "$RN" 2>/dev/null \
    && say "ok   $c10: installed = the expected $(wc -w <<<"$SET"); admitted = $(for a in $ADM; do printf '%s ' ${a:0:8}; done); 79c5ecf2 ${M7[$c10]:0:12}$([ $MODE = apply ] && echo ", 52156652 ${MN[$c10]:0:12} (= 63's)")" \
...
[ $ok = 1 ] && say "RS-5 $MODE ACCEPTED" || { say "RS-5 $MODE NOT ACCEPTED$([ "$MODE" = apply ] && echo ': rollback = rs-5.sh rollback, then rs-5-accept.sh rollback')"; exit 1; }
