# rs-4, ON nan as root (rs-4.sh feeds it to bash -s): the relay's predictor gets release 79c5ecf2 (musl init, aa6c985c).
# MODE=apply: the api-relay.env's two lines equal to $DEST/predict-lines.before.env become $DEST/predict-lines.env
#   (PREDICT_RELEASES + 79c5ecf2, every other release kept; DOMAIN_RELEASES = 79c5ecf2);
# MODE=rollback: the reverse (DOMAIN_RELEASES back to a4f22748; 79c5ecf2 no longer installed).
# Line-wise: every other line of the env file stays byte-identical, the file stays 0600 root, then ONE api-relay restart.
# Fails closed before its first write, and prints no env value. The release stays OFF (refused if any release setting is on).
set -euo pipefail
...
n=$(grep -cE '^SECRETS_(ATTESTED_RELEASE|RELEASE_DEPLOYMENTS|RELEASE_SIGNING_KEY_FILE|RELEASE_MIN_TCB|RELEASE_VMPL)=' "$ENVF" || true)
[ "$n" = 0 ] || { echo "REFUSING: $n release setting(s) on: this step runs with the release OFF"; exit 14; }
...
echo "nan: rs-4 $MODE: backup $BAK; the two lines replaced (now sha $(cut -c1-12 <<<"$(sha256sum < "$TO")")); api-relay restarted: invocation ${inv0:0:12} -> $inv1, NRestarts 0, MemoryMax 1536M; the release OFF"
