# rs-7, ON nan as root (rs-7.sh feeds it to bash -s): rs-5-remote.sh with its names changed, nothing else.
# MODE=apply: the api-relay.env's two lines equal to $DEST/predict-lines.before.env become $DEST/predict-lines.env
#   (PREDICT_RELEASES + f7888d86, every other release kept; DOMAIN_RELEASES = 52156652,f7888d86); MODE=rollback: the reverse.
# Line-wise: every other line stays byte-identical, the file stays 0600 root, every release setting line byte-identical
# (the release is ON for the listed deployments), then ONE api-relay restart. Fails closed before its first write; prints no value.
set -euo pipefail
...
BAK=$ENVF.bak-rs7-$MODE-$STAMP; cp -p "$ENVF" "$BAK"; chmod 600 "$BAK"
NEWENV=$ENVF.rs7-new
...
echo "nan: rs-7 $MODE: backup $BAK; the two lines replaced (now sha $(cut -c1-12 <<<"$(sha256sum < "$TO")")); api-relay restarted: invocation ${inv0:0:12} -> $inv1, NRestarts 0, MemoryMax 1536M; the release settings unchanged (${REL0:0:12})"
