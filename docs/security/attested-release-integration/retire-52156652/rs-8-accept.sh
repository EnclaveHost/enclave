#!/usr/bin/env bash
# rs-8 (the RETIRE of 52156652) live acceptance. apply:
#   1. the api-relay restarted by rs-8 (a new invocation, 0 restarts after); the KAT PASS in THAT invocation;
#   2. accept.sh with ADMIT=f7888d86: the release-ON shape; ADMIT=52156652, 79c5ecf2 and a4f22748: REFUSED for every canary;
#   3. per canary: installed = {5c3561f9, 6f14ce75, f7888d86}, admitted = {f7888d86}, at 63's independent pin;
...
# rollback: rs-7's state (52156652 and f7888d86 both admitted).
...
MODE=${1:?usage: rs-8-accept.sh apply|rollback}; H=$(cd "$(dirname "$0")" && pwd)
...
case $MODE in apply) ADM="$RF"; SET="$R5 $R6 $RF"; REFUSED="$RN $R7 $RA" ;; rollback) ADM="$RN $RF"; SET="$R5 $R6 $RN $RF"; REFUSED="$R7 $RA" ;; *) echo "apply|rollback"; exit 2 ;; esac
...
ok=1; bad() { say "RS-8 ACCEPT FAIL: $*"; ok=0; }
prop() { $NAN "systemctl show enclave-api-relay -p $1 --value"; }
as=$(prop ActiveState); nr=$(prop NRestarts); inv=$(prop InvocationID); inv0=$(cat "$RS/rs8-$MODE-inv0.txt" 2>/dev/null || true)
...
for a in $ADM; do
  out=$(ADMIT=$a bash "$H/../accept.sh" 2>&1); echo "$out" > "$RS/rs8-$MODE-accept-${a:0:8}.txt"; echo "$out" | grep -vE '^\.\. '
...
for a in $REFUSED; do
  out=$(ADMIT=$a bash "$H/../accept.sh" 2>&1); echo "$out" > "$RS/rs8-$MODE-refused-${a:0:8}.txt"; echo "$out" | grep -vE '^\.\. '
...
sys.exit(0 if ok else 1)' "$eg" "$SET" "$ADM" "$RN" "$([ $MODE = rollback ] && echo "${MN[$c10]}" || echo -)" "${MF[$c10]}" "$RF" 2>/dev/null \
    && say "ok   $c10: installed = $(for a in $SET; do printf '%s ' ${a:0:8}; done); admitted = $(for a in $ADM; do printf '%s ' ${a:0:8}; done); f7888d86 ${MF[$c10]:0:12} (= 63's pin)$([ $MODE = rollback ] && echo ", 52156652 ${MN[$c10]:0:12}")" \
...
[ $ok = 1 ] && say "RS-8 $MODE ACCEPTED" || { say "RS-8 $MODE NOT ACCEPTED$([ "$MODE" = apply ] && echo ': rollback = rs-8.sh rollback, then rs-6-accept.sh rollback')"; exit 1; }
