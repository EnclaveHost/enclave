// leased-attest.mjs (rs-12): three-line/leased-attest-next.mjs (bf GO @ f63b38ea: the scope hosts DERIVED from /enclaves, relay-
// verified mode snp) with ONLY LISTED's pins retargeted to the kept release R aee2059f and the message strings. rs-12's precondition (the retire of 5db18199), CHIP-VERIFIED: EVERY release-listed deployment that holds a LIVE LEASE (from
// the ledger: running-ness is never judged by whether a guest happens to answer) runs on aee2059f at its pinned measurement,
...
//   - the launch MEASUREMENT equals the pinned KEPT-release value (R aee2059f: 63's and bf's independent pins for the canaries, bf's
//     and mine for api-mcp-adapter; mine for d9798e4c, a77d0c57 and 7ae476a3 - no bundle on the host until they first launch).
...
// [deployment id, AppID, the aee2059f measurement, canary?] for every release-listed deployment (the 7 of 09-26 03:02Z)
const LISTED = [
  ["0x0ddbd82423a22883aca0862dc30f7320337e451bc126455cbe4d7846972c2e76", "d2c4dfc0ec475910aa509d1045ae4f2997346c1cd666a167cd5fc959c036aa24", "3facefd88284eac612f9b4e71ee35fc7b200d74f352c4d8d20e8b35e4a9599a456b88716d3d4f0bf579ee156c0cb57f4", true],
  ["0x395bed3e2e24efa02ba9dfed4aa8e081b064e7b5652b3e6474f11c21ae7f1595", "9c3d10f1450e17bc6a21478723193ef7e3da409afe353e264714cb801d180d45", "8a291bbf99818bdb6339ee34020d194d1de385c9f4597db60525c8cef0cc0318943e41d4d6d5ee4c3a98c4972b279504", true],
  ["0x4e62e60da567ca6c0b35f818192813e082149e738ad27204b5f074ed8adc6c1e", "9c3d10f1450e17bc6a21478723193ef7e3da409afe353e264714cb801d180d45", "8a291bbf99818bdb6339ee34020d194d1de385c9f4597db60525c8cef0cc0318943e41d4d6d5ee4c3a98c4972b279504", true],
  ["0xa69dcbbae66ac6ca71784d56209b1039142480ec97e0c8a3fd9cc658d969ed77", "94c04c0edb6b4ca11b9bd0b6e4adfa98afdfa04692e6c10af79755e6db0ba0f2", "5be511856afd91c2b19e9eb8ca8057682adfdc1fe400809a7bd71db3f4db904264daa7e37e029ae269a0a41badf34d0d", false],
  ["0xd9798e4ccd0c8402d0042000513fc6bc14616043d96dff3368080a21a1abbb9a", "e95a23b61e99c631e59deb0faa1b3cb4cc435131c03eba3e3f2a47dcddf9da37", "ee2c969030e139732d62a2b472a891213e9dcbe719d3687594f4c124ab46726176f2e9bd1b7d7ac82a5df945a4f761cd", false],
  ["0xa77d0c577c1ca48510ff72545f9e050dc7d1fc9c6d1129f056494a5190cb8371", "c8a709d9cfcfdcf1c5137befc264bdbe80f81e66538015d8d184260aedae7f5b", "2fb5058b679ff1c5e2d88f553a095c2f63634f9623c137a8f665a1ed7dc7e43bbfcd016a315b5e2f6c8df987d2c0b0a8", false],
  ["0x7ae476a3a1e4b0b144248075ff6656a0a10c3ae4cea8b6e4ad2b59dd8989ce33", "f187928c68071bf05ff50de307479a3cdbfad7b96c00ebcb6b2caa6424d52189", "6be3838b8f4e47d71a03fa097d0217db89f65bb69fc5372a21ca0ef964c6c00dae9a30ad9cc07a9c002b7195e99e6a3a", false],
];
...
for (const id of live) if (!LISTED.some(([x]) => x === id)) { console.log(`FAIL ${id.slice(2, 10)}: listed on the relay but no aee2059f pin here`); all = false; }
// SCOPE (enclave-bf's REQUIRED addition, enclave-87): the retire removes 5db18199 from the prediction AND certificate sets, so
...
try {
  ...
  if (JSON.stringify(a) !== JSON.stringify(b)) { console.log("FAIL the two RPCs disagree about the leases on the SNP hosts"); all = false; }
    ...
    if (extra.length && !process.env.OVERRIDE) { console.log(`FAIL scope: ${extra.length} deployment(s) leased on a verified SNP host are NOT release-listed (a guest of theirs on 5db18199 would lose its certificate): ${extra.map((x) => x.slice(0, 10)).join(", ")}; OVERRIDE=<reason> to proceed`); all = false; }
...
for (const [id, appId, want, canary] of LISTED.filter(([x]) => live.includes(x))) {
  ...
  try {
    ...
    else if (meas !== want) why = `measurement ${meas.slice(0, 12)} is not aee2059f's ${want.slice(0, 12)} (still on 5db18199?)`;
  } catch (e) { why = e.message; }
  console.log(`${why ? "FAIL" : "ok  "} ${label}: ${why || `leased; AMD-verified report of the serving guest (fresh nonce, TLS key bound), measurement ${meas.slice(0, 12)} = aee2059f's`}`);
  if (why) all = false; else if (canary) canariesOk++;
}
all = all && canariesOk === 3;
console.log(all ? "EVERY leased listed deployment chip-verified on aee2059f (the 3 canaries among them)" : "NOT every leased listed deployment is on aee2059f: rs-12 must not run");
