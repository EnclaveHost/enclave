# rs-6 (the RETIRE edit), ON nan as root (rs-6.sh feeds it to bash -s): rs-5-remote.sh with its names changed, nothing else.
# MODE=apply: the api-relay.env's two lines equal to $DEST/predict-lines.before.env become $DEST/predict-lines.env
#   (PREDICT_RELEASES = the KAT's two + 52156652; DOMAIN_RELEASES = 52156652);
# MODE=rollback: the reverse (rs-5's lines: 79c5ecf2 and a4f22748 installed again, both 79c5ecf2 and 52156652 admitted).
# Line-wise: every other line stays byte-identical, the file stays 0600 root, every release setting line byte-identical
# (the release is ON for the 3 canaries), then ONE api-relay restart. Fails closed before its first write; prints no value.
set -euo pipefail
...
BAK=$ENVF.bak-rs6-$MODE-$STAMP; cp -p "$ENVF" "$BAK"; chmod 600 "$BAK"
NEWENV=$ENVF.rs6-new
...
echo "nan: rs-6 $MODE: backup $BAK; the two lines replaced (now sha $(cut -c1-12 <<<"$(sha256sum < "$TO")")); api-relay restarted: invocation ${inv0:0:12} -> $inv1, NRestarts 0, MemoryMax 1536M; the release settings unchanged (${REL0:0:12})"
