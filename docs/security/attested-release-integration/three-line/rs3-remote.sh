# rs3-remote.sh (rs-9 admit / rs-10 retire, enclave-87's hard order 09-26), ON nan as root (the rs-N wrapper feeds it to bash -s):
# rs-8-remote.sh for an EXPLICIT certificate set - the THREE release lines (PREDICT_RELEASES, DOMAIN_RELEASES, CERT_RELEASES)
# equal to $DEST/predict-lines.before.env become $DEST/predict-lines.env (MODE=apply) or back (MODE=rollback), line-wise wherever
# each line sits in the file, then ONE api-relay restart. consistent() (enclave-5d's M1) refuses, before any write, a result where
# NOT (admit ⊆ installed AND cert ⊆ installed AND admit ⊆ cert). Every other line stays byte-identical; the file stays 0600 root;
# every release setting line byte-identical. Fails closed before its first write; prints no value.
set -euo pipefail
...
KEYS="SECRETS_RELEASE_PREDICT_RELEASES SECRETS_RELEASE_DOMAIN_RELEASES SECRETS_RELEASE_CERT_RELEASES"
...
for f in "$FROM" "$TO"; do
  [ "$(cut -d= -f1 "$f" | tr '\n' ' ')" = "$KEYS " ] || { echo "REFUSING: $f is not exactly the three lines"; exit 12; }
done
...
BAK=$ENVF.bak-rs3-$MODE-$STAMP; cp -p "$ENVF" "$BAK"; chmod 600 "$BAK"
NEWENV=$ENVF.rs3-new
( umask 077; awk -v f="$FROM" -v t="$TO" '
    ...
    END { exit (c == 3 ? 0 : 3) }' "$ENVF" > "$NEWENV" ) || { rm -f "$NEWENV"; echo "REFUSING: the line-wise edit did not replace exactly 3 lines"; exit 17; }
[ "$(stat -c '%a %U' "$NEWENV")" = "600 root" ] && [ "$(wc -l < "$NEWENV")" = "$(wc -l < "$ENVF")" ] \
  && [ "$(diff "$ENVF" "$NEWENV" | grep -c '^[<>]')" = 6 ] \
  && [ "$(diff "$ENVF" "$NEWENV" | sed -n 's/^> //p' | sort | sha256sum)" = "$(sort "$TO" | sha256sum)" ] \
  && [ "$(relset "$NEWENV")" = "$REL0" ] \
  || { rm -f "$NEWENV"; echo "REFUSING: the edited file is not the old one with exactly the three lines replaced"; exit 18; }
...
echo "nan: rs3 $MODE: backup $BAK; the three lines replaced (now sha $(cut -c1-12 <<<"$(sha256sum < "$TO")")); api-relay restarted: invocation ${inv0:0:12} -> $inv1, NRestarts 0, MemoryMax 1536M; the release settings unchanged (${REL0:0:12})"
