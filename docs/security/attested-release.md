...
`GET /v1/expected-guest?id=0x…` answers `{id, catalogRef, appId, images: [{release, runtimeId, measurement, releaseAdmitted}]}`: the guest the deployment's CONFIRMED record must run, predicted over every INSTALLED domain release (`SECRETS_RELEASE_PREDICT_RELEASES`; installing a release is itself the reviewed act, so there is no second allowlist; `releaseAdmitted` marks those in `SECRETS_RELEASE_DOMAIN_RELEASES`, the only ones the release itself admits). It is the per-app certificate gate's independent trust root (enclave-d1, GUEST-POOL-ROLLOUT row 6): a guest matches only if its AppID equals `appId` and its (measurement, runtimeId) is one image's pair. Public (AppIDs and measurements are derivable from the chain), GET only, its own per-IP rate, independent of the secrets store, `SECRETS_ATTESTED_RELEASE` and `SECRETS_RELEASE_DEPLOYMENTS`. Only for a PUBLIC deployment holding a live lease (403 `not_public`, 409 `not_leased`), so a public request can start a prediction only for an approved version of a public, leased deployment; it waits at most 3 s (then `503 warming`, `Retry-After`, `retryAfterSec`); 404 for an unknown or non-catalog deployment. The known-answer test runs at relay start whenever the predictor is configured.
