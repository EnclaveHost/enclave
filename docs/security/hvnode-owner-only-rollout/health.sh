...
curl -sS -m 15 "$API/v1/secrets/release-status?id=0xa69dcbbae66ac6ca71784d56209b1039142480ec97e0c8a3fd9cc658d969ed77" | grep -q '"listed":false' || bad "a69dcbba not listed:false"
out=$(ADMIT=${ADMIT:-79c5ecf24eb48a70e2bb20f4bca684b4d5e3c7700f9bf9d38735c19509898ce4} bash $HOME/Projects/enclave-release/docs/security/attested-release-integration/accept.sh 2>&1)
[ "$(grep -c '^ok   0x' <<<"$out")" = 3 ] && [ "$(grep -c '^FAIL' <<<"$out")" = 1 ] && grep -qx 'FAIL release-ticket answered 403, expected 503' <<<"$out" \
  && echo "release: listed x3 (a69dcbba not), accept.sh = exactly 'release ON'" || { echo "$out"; bad "accept.sh is not exactly 'release ON'"; }
