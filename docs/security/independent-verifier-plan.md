...
8. Client 0.1.0 has a persistence gap (policy acceptance persisted only after the exchange; no cross-process
   ...
   compare-and-swap, Web Locks in the extension, nothing sent on a failed commit) is agreed; the gap stays open until
   0.2.0 is pinned and that suite passes unchanged.
