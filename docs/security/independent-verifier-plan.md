...
| all | (precondition) | the live differential daily job green for 14 consecutive days with zero `disagree`, its three jobs included (section 10.9: the differential, the positive control, the provenance parity); the TUF refresh job green for two cycles | NOT MET: day 1 of 14. Run 36104580079 (2026-09-25 06:49Z, dispatched once because the 06:17Z schedule had not fired by 06:48Z) green in all three jobs: the differential agree-refuse on the signed-index path; the control agree, ours verified with no omission on Tinfoil's Genoa host, matched v0.0.155; the parity agree across all five legs (Chrome 153 on the runner), the served bundle this commit's. The TUF job green on dispatch (36097826591) after its fix; its scheduled cycles have not run |
