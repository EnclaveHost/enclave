...
| `verifier-pvm-client-update` (6 + 1 known finding, built CLI, strict command) | update manifests signed by the lab release key and countersigned by the lab policy key, artifact downloads held as barriers: a valid update stages its exact bytes and the state records it, tampered staged bytes are reported; a wrong countersignature, a stranger's release key and a non-newer version are refused with nothing on disk; two stagers of different versions in both completion orders (the older never replaces the newer); a policy-key rotation by signed `nextPolicyKey` after which the retired key's policies and a rollback under it are refused; a release-key rotation and a policy commit in both orders, neither losing the other's fields, then the successor signs and the retired key and a stranger do not; a storm of six concurrent updates and six policy commits ends at the highest version and serial with every loser refused by the monotonic rule and one generation per commit. Known finding (todo, named by the strict command): a refused same-version re-stage replaces the staged file's bytes |
...
8. Client 0.1.0 has a persistence gap (policy acceptance persisted only after the exchange; no cross-process
   ...
   Two findings reported to the owner: a refused stager's verified file stays beside the client under its own
   version name, and a second manifest for the SAME version with other bytes is refused yet replaces the staged file
   first (`staged` then reports `bytesMatch: false`); the latter is a todo case the strict command names until fixed.
