...
8. Client 0.1.0 has a persistence gap (policy acceptance persisted only after the exchange; no cross-process
   ...
   strict command. Closed for the CLI as far as black-box cases reach; the extension's lock path and update races are
   covered by the owner's own tests, not here.
