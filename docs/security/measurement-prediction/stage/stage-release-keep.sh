#!/bin/sh
# stage-release-keep.sh (rs-N, approved by enclave-87 09-26): stage-release.sh's copy with TWO changes, nothing else:
...
[ "$(cut -d= -f1 "$BEFORE_LINES" | tr '\n' ' ')" = "SECRETS_RELEASE_PREDICT_RELEASES SECRETS_RELEASE_DOMAIN_RELEASES " ] || { say "REFUSED: $BEFORE_LINES is not exactly the two lines"; exit 2; }
...
printf '%s\n%s\n' "$old_rel,$ID=$DEST/release" "SECRETS_RELEASE_DOMAIN_RELEASES=$keep,$ID" > "$DEST/predict-lines.env"
...
out.pass = k.ok && !p.problems.length && r.ok && im.length === 2 && of(id).length === 1 && of(id)[0].measurement === want
  && of(keep).length === 1 && of(keep)[0].measurement === wantKeep;
