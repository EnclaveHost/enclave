...
printf '%s\n%s\n' "SECRETS_RELEASE_PREDICT_RELEASES=$new_pairs" "SECRETS_RELEASE_DOMAIN_RELEASES=$KEEP" > "$DEST/predict-lines.env"
...
const keep = process.env.KEEP, installed = o.releases.map((r) => r.id).sort();
out.crossChecks = [];
