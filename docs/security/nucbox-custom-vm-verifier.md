...
shipped on `relay/deploy.sh`'s list and imported STATICALLY by `relay/tunnel.js` (so a relay deploy without it crash-loops,
which the deploy-closure guard prevents); the switch, not the import, keeps it off.
...
One more condition, found by enclave-d1's readiness review (2026-09-25, READINESS.md U7) and checked here against main.
The relay's evidence rule (`computeEligible`) governs PLACEMENT only. It decides `servingEnclaves()`, the claim hints
and the fleet's capacity. Two other paths ignore it:
- routing: `xOwnerOf`, then `runnerEndpointOf`, sends a deployment's /x and app-subdomain traffic to whichever LIVE row
  holds its lease on the ledger;
- certificate issuance: `relay/certs.js` issues the app hostname's certificate to the registered operator's signed CSR
  from the live lease holder.
Neither asks whether that row is eligible. So with the switch ON, an hv-node row that holds a lease is routed its
traffic and can obtain a publicly trusted certificate for it. For a stranger's deployment, the only guard would be the
node's own claim policy (owner-only), which is a HOST statement. The chain's claim rules check no TEE evidence.

So before the switch is turned on, the relay itself must restrict routing and certificate issuance for a row that is
not `computeEligible` to deployments whose OWNER is that row's registered operator (owner-only, lab serving), or refuse
them outright.

This is not specific to hv-node. Every live row that is not eligible and holds a lease is treated the same way today:
token-attached tunnels (mode "") and AVF tunnels outside the inference lane. It is a platform decision for the relay's
owner, reported to Steven, and nothing here changes it.
...
- treat `"keys"` (the vTPM's AK and EK: the vTPM state is host-readable on this box, enclave-5d's finding) and
  ...
  On this box every partition's guest state is a byte-copy of ONE master VMGS (`type1.vmgs`, 4f051697..., 4194816
  bytes). enclave-d1 MEASURED it read-only (2026-09-25 10:11:48Z; evidence 3e3ad330, pkg-accept-b7ba7731 README). The
  4 MiB body is all zero; the only 57 non-zero bytes are the 512-byte fixed-VHD footer (`conectix`); and there is no
