# rs-8 (the RETIRE of 52156652), ON nan as root (rs-8.sh feeds it to bash -s): rs-5-remote.sh with its names changed, nothing else.
# MODE=apply: the api-relay.env's two lines equal to $DEST/predict-lines.before.env become $DEST/predict-lines.env
#   (PREDICT_RELEASES = the KAT's two + f7888d86; DOMAIN_RELEASES = f7888d86);
# MODE=rollback: the reverse (rs-7's lines: 52156652 installed and admitted again beside f7888d86).
...
BAK=$ENVF.bak-rs8-$MODE-$STAMP; cp -p "$ENVF" "$BAK"; chmod 600 "$BAK"
NEWENV=$ENVF.rs8-new
...
echo "nan: rs-8 $MODE: backup $BAK; the two lines replaced (now sha $(cut -c1-12 <<<"$(sha256sum < "$TO")")); api-relay restarted: invocation ${inv0:0:12} -> $inv1, NRestarts 0, MemoryMax 1536M; the release settings unchanged (${REL0:0:12})"
