#!/usr/bin/env bash
# If main moved before the window: re-cut the 2 reviewed commits onto the current main (cherry-pick), require the SAME
...
git -C "$WT" push -q --force-with-lease origin HEAD:relay/prewarm-pacing
