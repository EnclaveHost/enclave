#!/usr/bin/env bash
# (as pace-recut.sh) If main moved before the window: re-cut the 2 reviewed commits onto the current main (cherry-pick), require the SAME
# patch-ids and file list, and that main changed nothing under relay/, site/ or scripts/ since REVIEW_BASE; force-push the
# window branch; update PC in lib.sh. Anything else refuses: a re-review.   usage: og-recut.sh <worktree>
set -euo pipefail; source "$(dirname "$0")/lib.sh"
WT=${1:?usage: og-recut.sh <worktree>}
...
git -C "$WT" push -q --force-with-lease origin HEAD:relay/owner-grace
sed -i "s/^PC=[0-9a-f]\{40\}/PC=$new/" "$H/lib.sh"
say "re-cut onto $(git -C "$WT" rev-parse --short origin/main): PC ${new:0:12} (the same 2 reviewed patches)"
