...
| Enclave Shield is Enclave's protected GPU-offload mechanism for any GPU not supporting or not operating in confidential-computing mode; masked inputs, verified results, never plaintext; required even beside a confidential CPU when the card is outside the boundary; a CC-mode card sits inside the boundary and does not depend on Shield | index lede, pillar card and contract-section note, Develop > Architecture #shield (aliases #shielded, #masked-offload) and the level table, host page, ticker, footer | owner's definition 2026-09-23; `docs/shielded-inference.md`, `shielded/README.md`, `shielded/worker-cuda/worker.cu`, `wasm/ggml-shielded/*`; the attestation document's GPU report (`supervisor.js` `ccMode`) is how a card inside the boundary is evidenced to clients | future tense for what is not running; "the engine runs on one self-hosted confidential server ... not on the hosted fleet"; numbers withheld |
| The gate reads the card's protection mode, never teeCpu, the host OS or the CPU; a card outside the boundary is neither advertised nor routed until its Shield evidence verifies; fail-closed | architecture #shield note, index note, host page | `relay/api-relay.js` `gpuModeOf` (confidential / shield / none from what the box presents about its card), `shieldVerified` (the masked-offload proof block, accepted only from a box whose word the relay accepts at all), `gpuSellable` applied to `pick()`, `aggregateAvailability` and the `/enclaves` totals, with `gpuMode` / `gpuSellable` / `gpuIneligible` on rows; mirrored in `site/js/core/pricing.js` `gpuModeOf` / `gpuSellableOf` (ranking, placement messages) and the fleet panel (no GPU pool drawn for an unsellable card) | tests: `test/api-relay.test.mjs` (a confidential box with a shielded card without proof: serving, card not sellable, GPU totals exclude it; with proof: included; a non-TEE carded box: card not sellable), `test/tenant-compute-eligibility.test.mjs`, `test/pricing.test.mjs` fixtures carry the proof block; a relay-verified Shield contract is still to come |
| The host isolation layer for machines without a confidential CPU is a separate, unbranded requirement: OS-neutral, VBS-like, Windows (VBS/Hyper-V, paravisor direction) and Linux (only if it enforces the equivalent contract), no software-only fallback, no owner-cannot-inspect guarantee, no machine listed until a backend passes the acceptance tests; Shield alone never makes such a host eligible for tenant apps | index pillar and contract cards, Develop > Architecture #contract h3, host page | `windows/vbs/REPORT.md`, `windows/vbslike/README.md`, `isolation/DESIGN.md` T0+ row, `isolation/contract`; enforced by `computeEligible` (tenant-compute eligibility is the CPU's evidence, separate from `gpuSellable`) | |
...
## Brand migration: Enclave Shielded to Enclave Shield (2026-09-23)

Every human-facing and explanatory use of the product name was renamed across the site pages and
components, PRODUCT.md, README, this record, the relay's policy comments and ineligible-reason copy,
and the test descriptions and expectations. In the same pass the brand's scope was corrected to the
GPU-offload mechanism only (the host isolation layer is a separate, unbranded requirement). The
...
Intentional remaining occurrences of the old form, all outside the product surface:

| where | why it stays |
|---|---|
| `shielded/anchor/avf/AndroidManifest.xml`, `payload/ggml-tpu.cpp`, `tpu/worker/tpu_worker_jni.cc`, `tpu/make_graphs.py`, `tpu/make_graphs2.py`, `shielded/anchor/optee/ta/user_ta_header_defines.h` | build inputs of the phone anchor app and the OP-TEE spike, owned by the pVM session; the APK label and TA description are part of measured artifacts whose codeHash the pVM CPU admission pins, so the rename belongs to that session's next build, not to a site branch |
| `docs/research-archive-2026-09/**` | a historical archive whose index says it is not a current statement; the name there refers to the 2026-09 phone-anchor and 27B campaigns as they were called at the time |
| `metal/systemd/enclave-shielded-worker.service` (`Description=Enclave shielded GPU worker`) | the masked-offload worker's systemd unit: "shielded" is the adjective for that worker, an operator-facing compatibility surface (unit name, shared-memory paths), not the brand |
| `PRODUCT.md` and this record | the two places that document the rename itself |
