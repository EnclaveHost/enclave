...
| 4b' | The production BUNDLE (5d's INSTALL.md per release) and enclave-63's guarded scripts, v4 at 483c5579 (evidence/s4-2026-09-25/): s4-install.sh (INERT), s4-guestd-install.sh (1c: separate caches + an independently reproduced sha, memory gate, capped scope), s4d-adoption-preflight.sh (read-only: the new tree's judge verifies all 3 canaries as adoption will; dry run PASSED on the installed tree, with a negative control), s4d-apply.sh / s4d-rollback.sh (4d, a one-time rollback token), s4-abandon.sh <id>. Codex authorized the INERT install after e3's review; 0839ac3a installed that way; a4f22748 installs beside it after review, then 0839ac3a is abandoned | enclave-5d + enclave-63 | enclave-e3 (v4 in review) |
