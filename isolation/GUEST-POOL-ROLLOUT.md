...
| 9 | 4d (s4d-apply.sh): guestd.<floor merge> with `-isolation iso-17e182a8 -release -legacy-isolation iso-03be27d6 -instance-prefix gd -guest-host-floor-mib 16384`, the budget 65536/16 unchanged. Preconditions scripted: the adoption preflight for THIS binary < 2 h old, the legacy-path check for THIS binary, 1d again, vsock 9443/9444 bind-probed free, no lab, headroom; post-checks include pool.host.unreadUnits == 0 (the d36e8da7 reader works on the box). Needs: the floor merge (5d, after e3's OK of d36e8da7) on the final image commit, the legacy re-run with that guestd, e3's OK of v4, and Codex's go | enclave-63 | enclave-5d, enclave-e3, enclave-d1 |
