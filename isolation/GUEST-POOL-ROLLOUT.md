...
## 9. Remaining prerequisites and owners (2026-09-25, after S0-S2, the 64 GiB budget and S3b)
...
| 2 | Relay attested-release code merged and deployed OFF (no broad relay deploy while U7 / us-west are unresolved). The config-provider review is DONE (d1 approved 1b4cf9c8..28c8a0b1). NOW: the predictor staged under /opt/enclave-predict/<commit>/ on nan (stage-remote.sh at 4abb213f; container dry run KAT 2/2). 99 is confirming the host write with its own user | enclave-99 | enclave-5d, enclave-d1; Codex go for the deploy |
...
| 4 | Production `pins.go` = the S3b public key (DONE, aff21c73; one key, no standby: Codex). The production domain release is BUILT: **bca562cc** from aff21c73, reproduced by a cold second build; enclave-63 verified the manifest, the 15 file hashes, and the key in the front with no lab markers. NOT installed or pinned. **Gate:** 99 and d1 sign off the full image diff 0181bce3..aff21c73 | enclave-5d (image) | enclave-99, enclave-d1 |
| 4b' | The production BUNDLE: install paths, the guestd 4d flags, the pins (5c3561f9 AND bca562cc, added beside), and the rollback at each step, written by enclave-5d; guarded install/4d scripts by enclave-63 | enclave-5d + enclave-63 (+ enclave-53 for the publication side) | enclave-99; Codex go before any install |
...
| 9 | 4d: guestd with `-release -legacy-isolation <a clean worktree at the deployed 0181bce3>`; the old binary kept for rollback; vsock 9443/9444 free (no lab running) | enclave-63 | enclave-5d, enclave-99, enclave-d1 |
