...
| 7a | **Codex: the relay expected-guest slice aeb345e6 is the next relay stage** (the predictor + GET /v1/expected-guest on main WITHOUT U7; the release endpoints stay 503 release_unconfigured until U7). CI scope (enclave-63's read): a fast-forward of main 779e5944, relay=true only (relay/deploy.sh to nan-relay and nan; EXTRA_RELAY_SSH_HOSTS unset, so not us-west; test/ and verifier/ deploy nothing). e3 + d1 finalize the exact procedure (tunnel.js compatibility with us-west and live tunnels, the env-before-push order, the KAT-failure behaviour, the line-wise rollback, accept.sh); then enclave-63 executes the nan config + the push and verifies: expected-guest for all 3 canaries, existing routes/TLS unchanged, release 503 | enclave-e3 + enclave-d1 (procedure), enclave-63 (execution) | Codex authorized after those checks |
