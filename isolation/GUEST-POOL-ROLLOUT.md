...
| 9b | **Codex (09-25, after accepting 4d): the LGPL decision keeps Enclave's LICENSE.** 5d rebuilds the small init against a non-copyleft libc (musl-style; verify the linked dependencies, keep notices and source; the toolchain must be pinned where guestd rebuilds the template, on warden-host). d1/e3 review the image, reproducibility, sentinel (stdio discard), legacy and predictor. Then enclave-63: the inert install beside iso-17e182a8 and a reviewed TREE SWITCH (s4t-apply/rollback/run at 6956d7e4: only -isolation moves, the same binary, detached, the adoption preflight) BEFORE any real release guest. Production keeps running meanwhile; no rollback artifact is deleted. The older glibc builds' publication stays held (53: glibc stays in the source bundle for the dynamically linked wasmtime; libgcc's GCC source too) | enclave-5d (init), enclave-63 (install + switch) | enclave-d1, enclave-e3; Codex go for the switch |
