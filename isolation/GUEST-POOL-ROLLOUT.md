...
| 3 | U7: **UNBLOCKED** (Steven unlocked the us-west key; `ssh us-west` as root). 5d's preflight rev 5 (d1-reviewed), e3's converged commit **2144fcb3** (main aeb345e6 + relay/ = fc90d6b5 + U7's tests; d1 APPROVED), enclave-63's execution scripts v2 (d1 PASS). **Step 1 DONE 21:59Z:** nan-relay tcp6/udp/dns carry ELIGIBILITY_API (inert), and the four data-plane env files are 600 root (dns.env's DNS_TXT_KEY was world-readable), nothing restarted. Next: the push (after e3's baseline suite diff), the nan/nan-relay smoke, us-west by hand, rollback per host (evidence/u7-2026-09-25/) | enclave-63 (executing), enclave-5d (preflight), enclave-e3 (commit) | enclave-d1 |
| 4 | Production `pins.go` = the S3b public key (DONE, aff21c73; one key, no standby). The production domain release is now **a4f22748** (a4f227482df4830ab69b52e38dc5d6e2abea9e5c5fb71f5469f0c30e6b1cb784) from **17e182a8** (5d, record 3770f796). Lineage: bca562cc → 31d117a9 (d1's proxy-log fix) → 0839ac3a (e3's HOST_DATA fail-open fix; INSTALLED inert 19:39Z, never activated, to be abandoned) → a4f22748: init gives the app /dev/null for stdin/stdout/stderr, so nothing a tenant prints reaches the host-read serial console (Codex's decision; 5d's sentinel lab with a positive control, 17e182a8's evidence). enclave-63 verified: release.json sha, 15/15 files, only template/init differs from 0839ac3a, front bd066066 unchanged, the production key, no lab markers; guestd, boot path, fwd and judge unchanged since d1a38994. **Gate:** e3 and d1 review 17e182a8 | enclave-5d (image) | enclave-e3, enclave-d1 |
| 4b' | The production BUNDLE (5d's INSTALL.md per release) and enclave-63's guarded scripts, v4 at 483c5579 (evidence/s4-2026-09-25/): s4-install.sh (INERT), s4-guestd-install.sh (1c: separate caches + an independently reproduced sha, memory gate, capped scope), s4d-adoption-preflight.sh (read-only: the new tree's judge verifies all 3 canaries as adoption will; dry run PASSED on the installed tree, with a negative control), s4d-apply.sh / s4d-rollback.sh (4d, a one-time rollback token), s4-abandon.sh <id>. Codex authorized the INERT install after e3's review: 0839ac3a installed 19:39Z, then **a4f22748 INSTALLED inert 20:09:57Z** (d1 + e3 approved; 1d reproduced and re-checked; adoption preflight dry run 3/3 on its tree; evidence 20818936). 0839ac3a ABANDONED 20:11:39Z (e3-approved abandon). Scripts v5 at e213739f (in e3's review): 4d runs only DETACHED through s4d-run.sh; the adoption preflight runs again right before the change | enclave-5d + enclave-63 | enclave-e3 (v4 in review) |
| 5 | Domain-release publication artifact: PACKAGED by enclave-53 at release/publication-0181bce3-e53 @ 31d0bb46, nothing published. a4f22748's tarball 7955e736... (built twice identically, = 5d's approved build; its third-party bytes equal 0181bce3's); the rollback 0181bce3's 732ef19f...; ONE shared corresponding-source bundle 7509205f... that must be served NEXT TO the tarballs (e3). **Two open decisions:** (1) e3's finding: LGPL-2.1 section 6 for the statically linked template/init (glibc) needs terms that permit modifying the combined work, which LICENSE section 2(a) does not; options: a LICENSE addition (**Steven**), or init built against a non-copyleft libc (engineering; changes init, so a new release and a guestd -isolation switch before any release guest exists); (2) where the tarball and the source bundle are hosted, together (**Steven/Codex**). Then e3, 63 and 5d review before publishing. Verifying clients (row 6) need this published | enclave-53 | the two decisions, then enclave-e3, enclave-63, enclave-5d |
| 6 | Trusted clients: TRACED and FIXED by d1. The gap: the per-app WebPKI certificate gate judged the guest against guestd's own measurement and AppID (pre-existing, also on the live canaries). Fix, reviewed on both halves: the supervisor half d1/guestcert-expected **059fdec5** (811db2b6 + 5d's must-fix: the prediction requirement follows the box's MEASURED backend; 12/12 mutations; APPROVED by 5d); the relay half e3's security/attested-release **93a97080 + c2fec2e5** (APPROVED by d1: public read of the relay's prediction, 403 private / 409 unleased before any build, per-IP bucket). Channel: verified WebPKI TLS to the measured SECRETS_API, origin https only. Renewal at 2/3 of a cert's life (~30 days' notice on a 90-day cert); a NEW guest key needs the endpoint at once. Verifying clients need the new release PUBLISHED beside 5c3561f9 (row 5) | **enclave-d1** | done; **dependency:** relay ≥ c2fec2e5 LIVE with the predictor wired and PREDICT_RELEASES = 5c3561f9 + 6f14ce75 + the new release BEFORE the row-8 image (which must carry 059fdec5, merged by 5d) ships; the relay deploy is behind U7 (row 3) |
| 7 | 4b: the relay's release ON with the full policy, SECRETS_RELEASE_DEPLOYMENTS = the canaries only | enclave-99 | **Codex** go |
| 8 | 4c: the ticket-fetch/cert-gate supervisor as the next measured node image. PREPARED 2026-09-25 (b5d990e5, evidence/s4c-2026-09-25/): the overlay from exactly **b18f8989** (the floor merge's supervisor half, d1's row-6 check 811db2b6+059fdec5, the launch-spec guard; ISOLATION_RELEASE unsettable), built twice identically with S2's command (AmdSev OVMF); prediction **e6b02e41...888c** = the hand sev-snp-measure; the 8 overlay files = b18f8989's. Compatibility (read-only): b18f8989's resume verdict against the live 4d guestd is claimable for all 3 canaries, with the floor verdict {16384, true}. Rollout: 4c-a (allowlist + api-relay restart, KAT + accept.sh again), 4c-b (dist + node CVM restart; 3 adopted resumes, 0 releases; availability with the floor verdict), both detached, with rollbacks. Its dependency (the relay predicts the canaries) is MET since 21:02Z | enclave-63 (rollout), enclave-5d (code) | IMAGE v2 **PASSED by enclave-d1**: b3109929 (b18f8989 + -ffile-prefix-map; v1 was checkout-path-dependent), prediction 8ab7a159...bf reproduced from 3 paths (8ae03c7a); d1 checks scripts v2 when 4c is next; then **Codex go** |
| 9 | **4d DONE 2026-09-25** (Codex's go): s4d-run.sh, detached, one guestd restart at 20:21:27Z to guestd.4e78ba80 (0ee7ed91) with `-isolation iso-17e182a8 -release -legacy-isolation iso-03be27d6 -instance-prefix gd -guest-host-floor-mib 16384`, 65536/16 kept; fresh adoption preflight 3/3 first; the 3 canaries adopted with their keys and unit PIDs; every post-check passed (pool.host floor 16384, pendingMiB 2175, unreadUnits 0; supports release+legacyImage, config/secrets/egress off); observe.sh 4d GATE PASSED 20:22-20:32Z (6 rounds, 612 s); before/after unchanged where they must be (evidence/s4-2026-09-25/4d/, 3aed8f7f). Rollback kept: s4d-rollback.sh to guestd.c42612c0 | enclave-63 | done; **VERIFIED read-only by enclave-e3** (sha256 of the running exe = its reproduction 0ee7ed91; the ExecStart; the journal's adoption, floor, release and legacy lines; the canary units and PIDs unchanged; 3 x 200 with valid TLS; availability 64/16, free 0.7) |
| 10 | 4e: canaries relaunched one at a time through a null-config release | enclave-63 / enclave-5d | enclave-99 |
| 9b | **The musl-init release 79c5ecf2 is LIVE as guestd's tree (2026-09-25).** rs-4 21:31:42Z (the relay predicts AND admits 79c5ecf2); the tree switch 21:46:34Z (-isolation iso-17e182a8 -> iso-aa6c985c, the same binary 4e78ba80, the 3 canaries adopted with their keys), observe gate PASSED 21:57:20Z (evidence/s4-2026-09-25/4t/, 4c756466). Rollback kept (s4t-rollback, then rs-4.sh rollback). NEVER DELETE the musl prefix | enclave-63 | done |
| 7a | **The relay expected-guest slice is LIVE and ACCEPTED (2026-09-25).** 20:56Z the predictor env (11 lines) and predict.conf on nan while the old relay ran (PID unchanged); 20:56:47Z fast-forward push 779e5944 -> aeb345e6; Deploy 36188755371 relay-only (jobs detect+relay, flags relay=true only, nan-relay only, not us-west); 20:59:08Z the new api-relay, KAT PASS 2/2; accept.sh: the 3 canaries predicted (be6b8644 under 5c3561f9, c068f423 under 6f14ce75), release-ticket 503 (release OFF), 404/422; MemoryPeak 731 MB. One mis-specified check (/v1/enclaves 200; it is 503 no_serving_enclave by design) was corrected and the acceptance re-run: ACCEPTED 21:02:02Z (evidence/relay-slice-2026-09-25/, 662bdcc1). Rollback kept (line-wise env, git revert). Next for the relay: e3 stages 79c5ecf2 on nan and adds it line-wise | enclave-63 (executed) | enclave-e3, enclave-d1 |
...
| 12 | S6: owner transaction payloads, prepared UNSIGNED by 5d at eba0b308 (isolation/restore/): 4 of Steven's 12 deployments fit the tier (a69dcbba, d9798e4c, a77d0c57 at 1%; 7ae476a3 at 2%, BLOCKED on us-west, row 3); setConfig writes only the envelope (+ isolation require), simulated as the owner on two RPCs at block 51787913; calldata sha256 per payload; --verify after signing. d1 verified all 4 on third RPCs and asked for a pre-sign staleness check and --verify tied to the tx input and the ConfigSet log (5d: written, in test). Signatures only AFTER canary acceptance (row 10) | enclave-5d prepares; **Steven** signs | Codex |
...
| 14 | The host-memory floor: d67b0020 + 1b5375c9 (e3 APPROVED) + **d36e8da7** (e3's follow-ups: a running guest counts R.mem minus its unit's memory.current, so the floor holds as SNP memory is faulted in lazily; no live number in any refusal, since the public claim-hint and the sweep log carried MemAvailable; a pure same-record helper from the parsed firewall; a starting held guest credited fully; an invalid pendingMiB = unknown). guestd -race green, supervisor 470/0, mutations 9/9 + 7/7. CODEX: 16384 MiB for THIS pool at 4d; default 0; no OOM/pricing change | enclave-63 | e3 APPROVED d36e8da7 and **ee8dbbcd**. MERGED by 5d: **4e78ba80** (onto 17e182a8, no template-input diff); guestd sha **0ee7ed91** reproduced by enclave-63 = 5d's; legacy-path re-run with that guestd PASSED (evidence b60873fa) |
