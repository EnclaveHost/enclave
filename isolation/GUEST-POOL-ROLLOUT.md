...
| 4 | Production `pins.go` = the S3b public key (DONE, aff21c73; one key, no standby: Codex). The production domain release is **0839ac3a** (0839ac3a859b7025abe261c56449d72f19fc8c4ed19ee50c63ab290718dfdb02) from **ecf02384**, 5d's record at 4e99148f. It SUPERSEDES 31d117a9 (d1a38994) and, before it, bca562cc. ONE fix: a FAILED HOST_DATA read (or one that is not 32 bytes) ends the domain; only a successful all-zero HOST_DATA means "no deployment" (e3's MEDIUM on 7de792bc; test release_decision_test.go). enclave-63 verified the release.json sha, 15/15 files, only template/front differs from 31d117a9 (front bd066066), the production key, no lab markers, and that nothing but the front, its test and the lab script changed since d1a38994: guestd and the boot path are byte-identical, so 5d's legacy-path PASS (023b06be) and the adoption analysis hold. The live 0181bce3 image never contained 7de792bc. **Gate:** d1 and e3 review ecf02384, and e3 re-runs the predictor cross-check | enclave-5d (image) | enclave-e3, enclave-d1 |
| 4b' | The production BUNDLE: install paths, the 4d flags, the pins (5c3561f9 AND 31d117a9, added beside), and the rollback per step (5d's INSTALL.md at 023b06be). Guarded scripts by enclave-63 (evidence/s4-2026-09-25/): s4-install.sh (tree + release + 1d, INERT), s4-guestd-install.sh (1c, the floor merge's binary), s4d-apply.sh / s4d-rollback.sh (the activation). **Codex authorized the INERT install** once enclave-e3's review and the reproduction checks pass. e3 APPROVED s4-install.sh 53fe6bdb + lib4.sh 93f212fd (v2) and s4-abandon.sh e95559cf. v3 (scripts-s4-v3.sha256) retargets them to 0839ac3a / ecf02384: the constants and one log string, nothing else; the install runs after e3 confirms v3 and row 4's gate | enclave-5d + enclave-63 | enclave-e3 |
...
| 6 | Trusted clients. TRACED by d1: (a) site/cli/mcp judge NODE CVMs only; (b) verifying clients (m2/client.mjs, verifier/live-domain-check.mjs) take the measurement as the caller's input, so their pin is PUBLICATION (row 5) plus the rule 5c3561f9 AND the new release; (c) the relay's predictor + SECRETS_RELEASE_DOMAIN_RELEASES is already a pin (INSTALL.md section 3); (d) **THE GAP (pre-existing, also on the live 0181bce3 canaries):** the per-app WebPKI certificate gate (supervisor-guestcert.mjs ensureGuestCert, via guestCertPass) judges the guest against guestd's measurement and guestd's AppID (rec._vmAppId from POST /vms), so a host running a modified image with the right HOST_DATA gets a browser-trusted certificate. FIX (d1, no new list): a relay read of predictionOf(confirmRow(id)), and the supervisor issues only if the appId matches and the guest's measurement is one of the predicted images. Required: an answer the host cannot forge (verified WebPKI TLS, or signed with the S3b key), no predictor build per request, fail-closed tests with mutations, the renewal window stated | **enclave-d1** (code), enclave-e3 (relay review), enclave-5d (supervisor review) | the relay half deploys with row 7 (behind U7); the supervisor half rides row 8's image, which must not ship before the relay endpoint is live |
