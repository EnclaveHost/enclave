 ...
 *   2. prints what the domain can see, as evidence for the isolation checks;
 *   3. drops to the domain's unprivileged uid and starts the two workloads: the runtime serving the
 ...
 * (namespaces, uids, cgroups), so the guest kernel is inside the app-vs-app TCB. That is WEAKER than
 * the VMPL separation described in PLAN.md section 1, and is not a substitute for it. SNP still
 * excludes the host from all of it.
 *
...
static void probe(const char *id, uid_t workload_uid) {
    struct stat st;
    ...
    printf("DOM%s probe workload_uid=%d sys=%d configfs=%d domains_dir=%d own_app=%d visible_pids=%d\n",
           id, (int)workload_uid, sysfs, cfg, doms, appok, procs);
}
 ...
 * must refuse. A domain that could obtain a report without being a registered domain would be able to
 * name any app it liked. */
static void probe_report_as_root(const char *id) {
     ...
     * that times out or is reset says nothing about what the monitor would have done, and reporting it
     * as "refused" would hide a real grant just as badly as reporting it as "GRANTED" invents one. */
...
static pid_t spawn(char *const argv[], uid_t uid) {
    pid_t pid = fork();
    if (pid == 0) {
        if (setgid(uid) != 0 || setuid(uid) != 0) { printf("DOM ERROR setuid: %s\n", strerror(errno)); _exit(127); }
...
int main(int argc, char **argv) {
    ...
    mount("proc", "/proc", "proc", 0, 0);              /* this domain's PID namespace only */
    mount("tmpfs", "/tmp", "tmpfs", 0, "size=64m,mode=1777");
    lo_up();
    probe_report_as_root(id);
