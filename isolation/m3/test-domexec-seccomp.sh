...
# domain's shape), with the null device on fd 3 as the monitor hands it. m2/app-seccomp-probe.c stands in for BOTH
...
run_all() {  # <domexec.c> <app-seccomp.h>
  rm -rf "$d/b" "$d/root" && mkdir -p "$d/b/m3" "$d/b/m2" "$d/root/plat/rt" "$d/root/run" "$d/root/tmp" "$d/root/proc"
  chmod 0755 "$d/root" "$d/root/plat" "$d/root/plat/rt"; chmod 1777 "$d/root/run"
  ...
  for mode in "app 64" "run 64 8080"; do
    rm -f "$d/root/run/"*.seccomp
    timeout 60 unshare --map-root-user --map-auto -mpfn -- sh -c "exec chroot '$d/root' /plat/domexec 7 1000 $mode 3<>/dev/null" > "$d/console.txt" 2>&1 || true
    for role in runtime front; do
      f="$d/root/run/$role.seccomp"
