#!/usr/bin/env bash
# 4e's five proofs for ONE canary after its owner restart (ENABLEMENT.md step 4), then its 10-minute observe; started
# DETACHED by e4-restart.sh (never sees the key). Any failure HOLDs (exit 25) with the proof named and changes NOTHING:
# with the release on, a failed release guest is relaunched by the supervisor (as a release guest again, backoff).
# Rollback, by a human: `rr-run.sh off` (release OFF line-wise: every respawn after it reads "unlisted" and runs the
# LEGACY image; a running release guest keeps serving until relaunched), then the owner's restart of that canary.
...
# detached through e4-restart.sh, or (E4_VERIFY=1, e4-verify.sh) in the foreground to verify a restart made another way;
# every step here only READS (guestd's /health + /vms, the node and relay journals, public TLS) except 4e's own state/
[ "${E4_VERIFY:-}" = 1 ] || grep -qE "^0::/.*/e4-proofs-$ID-[0-9]{8}T[0-9]{6}Z\.service\$" /proc/self/cgroup || { say "REFUSING: run through e4-restart.sh (or e4-verify.sh)"; exit 2; }
...
hold() { say "4e $ID HOLD: $* (nothing changed by this checker; rollback = rr-run.sh off, then the owner's restart of $ID)"; exit 25; }
...
say "4e $ID: new guest $NEWID running (the legacy one was $OLDID)"
...
M=$(expected_rel $FULL) && [ "$M" = "$RELM" ] || hold "proof 2: the relay's expected-guest image under 79c5ecf2 is now '${M:-none}'"
...
say "4e $ID proof 2 ok: release:true, attested, measurement ${RELM:0:16} = the relay's 79c5ecf2 prediction, new key ${NEWKEY:0:16}"
...
! grep -qE "$MARK" <<<"$C" || hold "proof 3: the app's own output ($MARK) is in the release guest's serial (the legacy control showed it)"
...
say "4e $ID proof 3 ok: DOM release (1 origin = the relay, config $CFGB bytes = the version's _media config) (envelope tag $ENV16) < app config $CFGB bytes < serving on ${SPKI:0:16}; no app output (the legacy control had it)"
# ---- proof 1: exactly one release for this id, to a verified guest on runtime ccadb38a; exactly one ticket handed over
RJ=$($NANX "journalctl -u enclave-api-relay --since '$T0 UTC' --no-pager -o cat") || hold "proof 1: the relay's journal on nan could not be read"
grep -iF "[secrets-release] $FULL:" <<<"$RJ" > $ST/$ID-relay-lines.txt || true
[ "$(grep -c . $ST/$ID-relay-lines.txt || [ $? = 1 ])" = 1 ] && grep -qE "released to a verified guest on .* \(runtime $RUNTIME" $ST/$ID-relay-lines.txt \
  || hold "proof 1: the relay's lines for $ID since T0 are not exactly one 'released to a verified guest … (runtime $RUNTIME…)' ($ID-relay-lines.txt)"
...
say "4e $ID proof 1 ok: one relay release to a verified guest (runtime $RUNTIME…), one ticket handed to $NEWID"
...
say "4e $ID proof 4 ok: certificate installed in $NEWID for key ${SPKI:0:16}, guest attested; no refusal ($(grep -c 'no certificate for' $ST/$ID-node-lines.txt || [ $? = 1 ]) retry line(s))"
...
say "4e $ID proof 5 ok: https://$HOST/ 200 via us-west, new serial $NS on key ${SPKI:0:16}"
...
PY
say "4e $ID: ALL FIVE PROOFS PASS; 4e's expected key for $ID is now ${NEWKEY:0:16}; the 10-min observe"
start=$(date +%s); end=$((start + 600)); round=0
while :; do
  round=$((round+1))
  if public_ok4 && relay_row_ok && check_guestd4 >/dev/null; then say "4e $ID observe r$round: 3/3 canaries 200 on 4e's keys, relay serving, guestd as expected"
...
say "4e $ID ACCEPTED: five proofs + the observe ($round rounds over $(( $(date +%s) - start )) s). Next canary: $(case $ID in 0ddbd824) echo 395bed3e;; 395bed3e) echo 4e62e60d;; *) echo none, 4e done: then 4b;; esac)"
