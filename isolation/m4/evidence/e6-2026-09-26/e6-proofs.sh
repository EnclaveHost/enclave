#!/usr/bin/env bash
# e6's proofs for ONE canary after its owner restart onto the HARDENED f7888d86 (derived from e5-proofs.sh v4, the restated
# proof 1 included): the five proofs against f7888d86 (the measurement = the pinned independent value = the relay's
# prediction), proof 6 (hookbin's HEAD: the console guard still withholds the body) and proof 7 (the hardening holds:
# yama ptrace_scope -> 2, user.max_user_namespaces -> 0, kernel.io_uring_disabled -> 2, no "DOM ERROR refusing to start"),
# then the 10-min observe. Started DETACHED by e6-restart.sh (never sees the key). Any failure HOLDs (exit 25).
set -euo pipefail; source ~/enclave-bench/pool-rollout-20260925/lib.sh; source ~/enclave-bench/s4c-20260925/lib4cc.sh; source ~/enclave-bench/release-on-20260925/lib-ro.sh; source ~/enclave-bench/e6-20260926/lib-e6.sh
trap '' HUP PIPE
...
grep -qE "^0::/.*/e6-proofs-$ID-[0-9]{8}T[0-9]{6}Z\.service\$" /proc/self/cgroup || { say "REFUSING: run through e6-restart.sh"; exit 2; }
...
hold() { say "6e $ID HOLD: $* (nothing changed by this checker; rollback = s7t-rollback.sh, which needs OVERRIDE=<reason> once any canary is accepted on f7888d86, then the owner's restart of $ID)"; exit 25; }
...
say "6e $ID: new guest $NEWID running (the legacy one was $OLDID)"
# ---- proof 2: guestd's view: release, attested, the relay's f7888d86 image = the pin
...
M=$(expected_rel $FULL) && [ "$M" = "$RELM" ] || hold "proof 2: the relay's expected-guest image under f7888d86 is now '${M:-none}', not the independently computed ${RELM:0:12}"
...
say "6e $ID proof 2 ok: release:true, attested, measurement ${RELM:0:16} = the independently computed f7888d86 value = the relay's prediction, new key ${NEWKEY:0:16}"
...
say "6e $ID proof 7 ok: $(grep -E '^DOM (yama ptrace_scope|user\.max_user_namespaces|kernel\.io_uring_disabled)=' <<<"$C" | sed 's/^DOM //' | paste -sd';' | sed 's/;/; /g')"
say "6e $ID proof 3 ok: DOM release (1 origin = the relay, config $CFGB bytes = the version's _media config) (envelope tag $ENV16) < app config $CFGB bytes < serving on ${SPKI:0:16}; no app output"
...
say "6e $ID proof 1 ok: $p1; one ticket handed to $NEWID"
...
say "6e $ID proof 4 ok: certificate installed in $NEWID for key ${SPKI:0:16}, guest attested; no refusal ($(grep -c 'no certificate for' $ST/$ID-node-lines.txt || [ $? = 1 ]) retry line(s))"
...
say "6e $ID proof 5 ok: https://$HOST/ 200 via us-west, new serial $NS on key ${SPKI:0:16}"
...
if [ "$ID" = 0ddbd824 ]; then
  ...
  say "6e $ID proof 6 ok: HEAD -> $hc; the serial withholds the body ($(serial_clean "$SER" | grep -m1 '^DOM front: unsolicited upstream response (' )), 0 'Unsolicited response' lines (a regression check: the console guard stays in the hardened front)"
fi
...
PY
say "6e $ID: ALL PROOFS PASS; 4e's expected key for $ID is now ${NEWKEY:0:16}; the 10-min observe"
start=$(date +%s); end=$((start + 600)); round=0
while :; do
  round=$((round+1))
  if public_ok4 && relay_row_ok && check_guestd4 >/dev/null; then say "6e $ID observe r$round: 3/3 canaries 200 on 4e's keys, relay serving, guestd as expected"
...
say "6e $ID ACCEPTED: five proofs + the observe ($round rounds over $(( $(date +%s) - start )) s). Next canary: $(case $ID in 0ddbd824) echo 395bed3e;; 395bed3e) echo 4e62e60d;; *) echo none: the hardened relaunch is done;; esac)"
