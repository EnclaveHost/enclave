#!/usr/bin/env bash
# The HARDENED-release relaunch for ONE canary (0ddbd824, then 395bed3e, then 4e62e60d), after the S7 tree switch: its
# preconditions (guestd on iso-b63c2def; the canary its 52156652 release guest; the relay predicting AND admitting
# f7888d86 for it = the pinned independent value), a snapshot, the OWNER's restart through the node base
# (api.enclave.host/v1/auth is full-service only; enclave-d1), then e6-proofs.sh DETACHED. The key ONLY from this
# process's environment as ENCLAVE_KEY. Run e.g.
#   bash -ic 'ENCLAVE_KEY="$ETH_AGENT_WALLET" ~/enclave-bench/e6-20260926/e6-restart.sh 0ddbd824'
# Derived from e5-restart.sh. Nothing here rolls back: a failed proof HOLDs (e6-proofs.sh).
...
systemctl --user list-units --plain --no-legend --all 'e4-*' 'e5-*' 'e6-*' 'rr-*-apply-*' 's4c*-apply-*' 's5t-apply-*' 's7t-apply-*' | grep -q . && { say "REFUSING: a rollout unit is active"; exit 2; }
X=$(systemctl --user show enclave-guestd.service -p ExecStart --value) && grep -Fq -- " -isolation $NEWT/isolation " <<<"$X" || { say "REFUSING: guestd's -isolation is not $NEWT (run the S7 tree switch first)"; exit 2; }
...
[[ "$RELM" =~ ^[0-9a-f]{96}$ ]] && [ "$RELM" != "$LEGM" ] || { say "REFUSING: no pinned f7888d86 measurement for $ID"; exit 3; }
M=$(expected_rel $FULL) && [ "$M" = "$RELM" ] || { say "REFUSING: the relay's f7888d86 prediction for $ID is '${M:-none}', not the independently computed ${RELM:0:12}"; exit 3; }
...
python3 -c "import json,sys; e=json.loads(sys.argv[1]); sys.exit(0 if e['status']=='running' and e['measurement']=='$LEGM' and e.get('release') is True else 1)" "$E" \
  || { say "REFUSING: $ID is not its running 52156652 RELEASE guest (measurement ${LEGM:0:12}) now"; exit 3; }
...
# no leak control here: the current guests already run the console guard (52156652); proof 6 stays as a regression check
...
PY
say "6e $ID: 52156652 release guest $OLDID (${LEGM:0:12}) -> f7888d86 predicted ${RELM:0:12}; certificate serial $OLDSERIAL; the OWNER's restart now (T0 $T0 UTC)"
set +e
...
say "6e $ID: enclave restart exited $rc: $(tr '\n' ' ' < $ST/$ID-restart.txt | cut -c1-200)"
[ $rc = 0 ] || { say "6e $ID: the restart was not accepted; nothing changed on the node? check guestd before retrying ($ST/$ID-before.json kept: remove it to retry)"; exit 4; }
U=e6-proofs-$ID-$(date -u +%Y%m%dT%H%M%SZ)
env -u ENCLAVE_KEY systemd-run --user --unit="$U" --collect -q -p Environment=PATH=/usr/local/bin:/usr/bin:/usr/sbin:/bin \
  --working-directory="$HOME" bash -c '"$0" "$1"; echo $? > "$2"' $E4/e6-proofs.sh "$ID" "$ST/$ID-proofs.rc"
say "6e $ID: the five proofs + the 10-min observe run DETACHED as $U (exit code -> $ST/$ID-proofs.rc); follow: tail -f $LOG4"
