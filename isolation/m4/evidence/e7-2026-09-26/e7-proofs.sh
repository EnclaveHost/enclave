#!/usr/bin/env bash
# e7's proofs for ONE canary after its owner restart onto 5db18199 (derived from e6-proofs.sh): the five proofs against
# 5db18199 (the measurement = the pinned independent value (e7-pins.sh) = the relay's prediction; the runtime = the one
# the relay predicted at the restart, before.json), proof 6 (hookbin's HEAD: the console guard still withholds the body),
# proof 7 (the e6 hardening still holds), and the NEW ones (enclave-87; fields from enclave-b4, 0c087de8):
#   proof 8, W^X AT ATTEST: guestd's verify.txt says VERDICT attested, RESULT gate=open, RESULT wx_coverage=runtime-covered,
#     and the attested runtime_selftest has wx=clean, runtime=R>=1 and its roles adding up to maps (on SNP: runtime + root);
#     the serial states "DOM runtime selftest exec_pages=allowed wx=at-each-attestation" and never "... FAILED ...";
#   proof 9, the app's seccomp: NOT REFUSED, NOT POSITIVELY ATTESTED: no "DOM ERROR the app's seccomp filter could not be
#     installed" line, and the app serves (implied by the measured dominit's exit-125 path and serving: a failed install
#     exits before exec, the app never listens, and the front never says DOM serving; enclave-b4);
#   the front uid: n/a on SNP (M3 only).
# Then the 10-min observe. Started DETACHED by e7-restart.sh (never sees the key). Any failure HOLDs (exit 25).
...
# detached through e7-restart.sh only; every step here only READS (guestd, the node and relay journals, public TLS)
# except 5e's own state/, and proof 6's one public HEAD request
grep -qE "^0::/.*/e7-proofs-$ID-[0-9]{8}T[0-9]{6}Z\.service\$" /proc/self/cgroup || { say "REFUSING: run through e7-restart.sh"; exit 2; }
...
hold() { say "7e $ID HOLD: $* (nothing changed by this checker; report to enclave-87. Rollback = s8t-rollback.sh with OVERRIDE=<reason> once any canary is accepted on 5db18199, then the owner's restart of $ID)"; exit 25; }
...
say "7e $ID: new guest $NEWID running (the legacy one was $OLDID)"
...
say "7e $ID proof 2 ok: release:true, attested, measurement ${RELM:0:16} = the independently computed 5db18199 value = the relay's prediction, runtime ${RTID:0:12} = the prediction, new key ${NEWKEY:0:16}"
...
say "7e $ID proof 7 ok: $(grep -E '^DOM (yama ptrace_scope|user\.max_user_namespaces|kernel\.io_uring_disabled)=' <<<"$C" | sed 's/^DOM //' | paste -sd';' | sed 's/;/; /g')"
...
say "7e $ID proof 8 ok: W^X at attest: VERDICT attested, gate open, wx_coverage=runtime-covered, self-test \"$WX\"; the front states wx=at-each-attestation, no FAILED"
...
say "7e $ID proof 9 ok: the app's seccomp filter: NOT REFUSED, NOT POSITIVELY ATTESTED (implied by the measured dominit's exit-125 path and serving: no install error, and DOM serving at line $sl); front uid: n/a on SNP (M3 only)"
say "7e $ID proof 3 ok: DOM release (1 origin = the relay, config $CFGB bytes = the version's _media config) (envelope tag $ENV16) < app config $CFGB bytes < serving on ${SPKI:0:16}; no app output"
...
say "7e $ID proof 1 ok: $p1; one ticket handed to $NEWID"
...
say "7e $ID proof 4 ok: certificate installed in $NEWID for key ${SPKI:0:16}, guest attested; no refusal ($(grep -c 'no certificate for' $ST/$ID-node-lines.txt || [ $? = 1 ]) retry line(s))"
...
say "7e $ID proof 5 ok: https://$HOST/ 200 via us-west, new serial $NS on key ${SPKI:0:16}"
...
if [ "$ID" = 0ddbd824 ]; then
  ...
  say "7e $ID proof 6 ok: HEAD -> $hc; the serial withholds the body ($(serial_clean "$SER" | grep -m1 '^DOM front: unsolicited upstream response (' )), 0 'Unsolicited response' lines (a regression check: the console guard stays in the 5db18199 front)"
fi
...
PY
say "7e $ID: ALL PROOFS PASS; 4e's expected key for $ID is now ${NEWKEY:0:16}; the 10-min observe"
start=$(date +%s); end=$((start + 600)); round=0
while :; do
  round=$((round+1))
  if public_ok4 && relay_row_ok && check_guestd4 >/dev/null; then say "7e $ID observe r$round: 3/3 canaries 200 on 4e's keys, relay serving, guestd as expected"
...
say "7e $ID ACCEPTED: proofs 1-9 + the observe ($round rounds over $(( $(date +%s) - start )) s). Next canary: $(case $ID in 0ddbd824) echo 395bed3e;; 395bed3e) echo 4e62e60d;; *) echo "none: the 5db18199 relaunch is done (tell e3: rs-10)";; esac)"
