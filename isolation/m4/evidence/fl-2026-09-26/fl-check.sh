#!/usr/bin/env bash
# One of Steven's apps' FIRST LAUNCH on 52156652, checked READ-ONLY (started by fl-watch.sh at its setConfig). Never acts,
...
# enclave-bf: once the S7 switch has run (its restart time recorded in state/s7-switched-epoch), a guest CREATED after it
# must be the hardened f7888d86; one on anything else means the switch did not take
if [ -r $FST/s7-switched-epoch ]; then
  SW=$(cat $FST/s7-switched-epoch); CA=$(python3 -c "import json,sys; print(int(json.loads(sys.argv[1])['createdAt']))" "$E")
  [[ "$SW" =~ ^[0-9]{10}$ ]] || hold "A: state/s7-switched-epoch is not an epoch"
  [ "$CA" -le "$SW" ] || [ "$GREL" = f7888d8690845cbb862c1fbcae0a22f5458fcb891de7d0d3ae31ea927536b7ca ] || hold "A: $GID was created after the S7 switch but runs release ${GREL:0:8}, not the hardened f7888d86"
fi
...
say "first launch $AID A ok: attested on release ${GREL:0:8}, measurement ${MEAS:0:16} = the relay's admitted prediction = expected-measurement.sh (that release) over its bundle; runtime ${RTID:0:12} (guestd = the relay's)"
