#!/usr/bin/env bash
# N2 (enclave-87 approved plan abb87c12): the metal-iso0 node image carrying the next chain rev's node half, from the clean
# merge 6845565a (isolation/node-image-n2 = N1's 2492e683 + 4cd26e58, tree c396ae32). build-n1.sh's command, the same
# pins, TWICE from TWO clean worktrees at DIFFERENT paths; hand measurement == manifest; the overlay and cpio diffs are
# taken against N1 (dist-iso-2492e683) and must be supervisor.js + supervisor-guestcert.mjs + judge.mjs (+ the manifest).
# Idle class under the same guard. INERT until N2-b.
set -euo pipefail
C=6845565a9dc0df5d8b469b8f2e74699c1aaa3444
N=~/enclave-bench/n2-20260926; EV=$N/build; B1=/home/steven/Projects/enclave-build-${C:0:8}; B2=$N/other/checkout/path/src
OUT1=/home/steven/enclave-prod/dist-iso-${C:0:8}; OUT2=$EV/dist-2; LIVE=/home/steven/enclave-prod/dist-iso-2492e683   # the diffs are against N1
...
[ "$(git -C /home/steven/Projects/enclave rev-parse $C)" = "$C" ] && git -C /home/steven/Projects/enclave merge-base --is-ancestor $C origin/isolation/node-image-n2 || { echo "$C is not on origin/isolation/node-image-n2"; exit 2; }
...
print("overlay vs N1 2492e683: differ =", sorted(p for p in set(lf)|set(af) if lf.get(p)!=af.get(p)))
print("images            :", a['images']['supervisor']['ref'][-16:], a['images']['wasmManager']['ref'][-16:], "(N1:", l['images']['supervisor']['ref'][-16:], l['images']['wasmManager']['ref'][-16:]+")")
PY
...
print("cpio entries: N1 %d, N2 %d; differing: %s" % (len(a),len(b),diff))
PY
echo "N2 build done $(date -u +%H:%M:%SZ)"
