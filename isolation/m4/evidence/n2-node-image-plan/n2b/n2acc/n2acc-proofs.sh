#!/usr/bin/env bash
# n2acc's proofs for hookbin's relaunch onto 5db18199 under the N2 node image: e7's proofs 1-9 unchanged (the same release,
# so the same pins and fields), plus N2-b's acceptance (plan abb87c12 §5, 5d's fixes A1-A3):
#   N2: the node attests N2 (fab9c6c7 / 6845565a) throughout, and its certificate line for the new guest reads
#       "guest attested" (N2's judge: 5db18199 named from the prediction, seccomp unstated, listed);
#   A1: IF a "[isolation] 0x0ddbd824: no certificate for 0ddbd824.app.enclave.host (the instance is starting); retry in"
#       line appears, every such line says 20s, NEVER 300s;
#   A2: REPORTED, not gated: the time from guestd first reporting the guest running (this checker's 5 s poll) to the
#       certificate install, and whether a starting retry was pending; the request goes out on the first pass at or
#       after serving (<= 21 s after it with a pending starting retry, else the next 60 s tick);
#   A3: every retry grep is anchored on the [isolation] certificate line ([bill] lines never count).
# Started DETACHED by n2acc-restart.sh. Any failure HOLDs (exit 25).
...
# detached through n2acc-restart.sh only; every step here only READS (guestd, the node and relay journals, public TLS)
# except 5e's own state/, and proof 6's one public HEAD request
grep -qE "^0::/.*/n2acc-proofs-$ID-[0-9]{8}T[0-9]{6}Z\.service\$" /proc/self/cgroup || { say "REFUSING: run through n2acc-restart.sh"; exit 2; }
...
hold() { say "n2acc $ID HOLD: $* (nothing changed by this checker; report to enclave-87. Node rollback = n2b-rollback.sh with OVERRIDE=<reason> (this canary's key moved))"; exit 25; }
...
say "n2acc $ID: new guest $NEWID running (the legacy one was $OLDID)"
# ---- proof 2: guestd's view: release, attested, the relay's 5db18199 image = the pin, the runtime = the prediction
...
M=$(expected_rel $FULL) && [ "$M" = "$RELM" ] || hold "proof 2: the relay's expected-guest image under 5db18199 is now '${M:-none}', not the independently computed ${RELM:0:12}"
...
say "n2acc $ID proof 2 ok: release:true, attested, measurement ${RELM:0:16} = the independently computed 5db18199 value = the relay's prediction, runtime ${RTID:0:12} = the prediction, new key ${NEWKEY:0:16}"
...
say "n2acc $ID proof 7 ok: $(grep -E '^DOM (yama ptrace_scope|user\.max_user_namespaces|kernel\.io_uring_disabled)=' <<<"$C" | sed 's/^DOM //' | paste -sd';' | sed 's/;/; /g')"
...
print(st)
PY8
) || hold "proof 8: the attested runtime self-test does not cover the runtime (wx=clean, exec_pages=allowed, runtime>=1, roles = maps, a known scope) ($ID-verify.txt)"
...
say "n2acc $ID proof 8 ok: W^X at attest: VERDICT attested, gate open, wx_coverage=runtime-covered, self-test \"$WX\"; the front states wx=at-each-attestation, no FAILED"
# ---- proof 9 (the app's seccomp, 5db18199): not refused; not positively attested
! grep -q "^DOM ERROR the app's seccomp filter could not be installed" <<<"$C" || hold "proof 9: the serial says the app's seccomp filter could not be installed"
say "n2acc $ID proof 9 ok: the app's seccomp filter: NOT REFUSED, NOT POSITIVELY ATTESTED (implied by the measured dominit's exit-125 path and serving: no install error, and DOM serving at line $sl); front uid: n/a on SNP (M3 only)"
say "n2acc $ID proof 3 ok: DOM release (1 origin = the relay, config $CFGB bytes = the version's _media config) (envelope tag $ENV16) < app config $CFGB bytes < serving on ${SPKI:0:16}; no app output"
...
say "n2acc $ID proof 1 ok: $p1; one ticket handed to $NEWID"
...
say "n2acc $ID proof 4 ok: certificate installed in $NEWID for key ${SPKI:0:16}, guest attested; no refusal ($(grep -c 'no certificate for' $ST/$ID-node-lines.txt || [ $? = 1 ]) retry line(s))"
...
say "n2acc $ID N2-b acceptance ok: the node attests N2; certificate for $NEWID judged 'guest attested' by N2's judge; A1: $ns starting retry line(s), all 20 s, no 300 s; A2 (reported): installed $A2 s after guestd first reported the guest running (install at $TINS); A3: [isolation]-anchored greps only"
...
say "n2acc $ID proof 5 ok: https://$HOST/ 200 via us-west, new serial $NS on key ${SPKI:0:16}"
...
if [ "$ID" = 0ddbd824 ]; then
  ...
  say "n2acc $ID proof 6 ok: HEAD -> $hc; the serial withholds the body ($(serial_clean "$SER" | grep -m1 '^DOM front: unsolicited upstream response (' )), 0 'Unsolicited response' lines (a regression check: the console guard stays in the 5db18199 front)"
fi
...
PY
say "n2acc $ID: ALL PROOFS PASS; 4e's expected key for $ID is now ${NEWKEY:0:16}; the 10-min observe"
start=$(date +%s); end=$((start + 600)); round=0
while :; do
  round=$((round+1))
  if public_ok4 && relay_row_ok && check_guestd4 >/dev/null; then say "n2acc $ID observe r$round: 3/3 canaries 200 on 4e's keys, relay serving, guestd as expected"
...
say "n2acc $ID ACCEPTED (N2-b's acceptance): proofs 1-9 + N2/A1-A3 + the observe ($round rounds over $(( $(date +%s) - start )) s). Next canary: $(case $ID in 0ddbd824) echo 395bed3e;; 395bed3e) echo 4e62e60d;; *) echo "none (hookbin only): N2-b is accepted; next S9";; esac)"
