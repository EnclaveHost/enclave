#!/usr/bin/env bash
# 4c v2: the node image from b3109929 = b18f8989 (5d's candidate: the floor merge's supervisor half, d1's row-6 cert check
# 811db2b6+059fdec5, the launch-spec guard; the 8 overlay files unchanged) + build-image.mjs's -ffile-prefix-map (d1: v1's
# measurement depended on the CHECKOUT PATH). Built with S2's exact command, TWICE, from TWO clean worktrees at DIFFERENT
# paths (of different lengths): the two must be identical, which is the proof of the fix. Inert.
set -euo pipefail
C=$(cat ~/enclave-bench/s4c-20260925/fix-commit.txt); B1=/home/steven/Projects/enclave-build-${C:0:8}; B2=/home/steven/enclave-bench/s4c-20260925/another/checkout/path/src; EV=~/enclave-bench/s4c-20260925/build-v2
