#!/usr/bin/env bash
# 4c-c-a: ADD the 4c image's measurement to the relay's METAL_ALLOWED_MEASUREMENTS on nan, beside 04e953a4 (pre-pool) and
# 10622d98 (S2, live), both kept for their soaks; ONE line changes; one api-relay restart (every tunnel node and the API
# blip; the predictor re-runs its KAT and warms again). A failure restores the ONE line. Run DETACHED via s4cc-run.sh a.
set -euo pipefail; source ~/enclave-bench/pool-rollout-20260925/lib.sh; source ~/enclave-bench/s4c-20260925/lib4cc.sh
trap '' HUP PIPE
grep -qE '^0::/.*/s4cca-apply-[0-9]{8}T[0-9]{6}Z\.service$' /proc/self/cgroup || { say "REFUSING: run 4c-c-a detached, through s4cc-run.sh a"; exit 2; }
check_prediction4c || exit 2
[ "$(allowlist)" = "METAL_ALLOWED_MEASUREMENTS=$ALLOW_NOW" ] || { say "REFUSING: the allowlist is not exactly 04e953a4,10622d98,8ab7a159"; exit 3; }
...
say "4c-c-a: adding ${NEWM:0:16} to the allowlist on nan (api-relay restart: every tunnel and the API blip)"
set +e; $NAN "set -euo pipefail
F=/etc/nan-relay/api-relay.env; B=\$F.bak-4c-\$(date -u +%Y%m%dT%H%M%SZ); OLD='METAL_ALLOWED_MEASUREMENTS=$ALLOW_NOW'; NEW='METAL_ALLOWED_MEASUREMENTS=$ALLOW_NEW'
...
grep '^METAL_ALLOWED_MEASUREMENTS=' \$F | awk -F'[=,]' '{print \"allowlist now:\", NF-1, \"entries\"}'" > $S4C/4cca-remote.txt 2>&1; r=$?; cat $S4C/4cca-remote.txt
set -e; [ $r = 0 ] || { say "4c-c-a FAILED on nan (4cca-remote.txt): the one line was restored"; exit 4; }
fail() { set +e; say "4c-c-a CHECK FAILED: $* -> rolling back 4c-c-a"; FORCE_ORDER="4c-c-a apply's own check (the node was not restarted)" $S4C/s4cca-rollback.sh; local rc=$?; [ $rc = 0 ] && exit 20; say "ROLLBACK FAILED rc=$rc: ESCALATE"; exit 24; }
wait_for 180 relay_row_ok || fail "metal-iso0 did not re-attach"
read -r am ao <<<"$(node_attested)" || true; [ "${am:-}" = "$OLDM" ] && [[ "${ao:-}" == b3109929* ]] || fail "the node no longer attests 8ab7a159 / b3109929"
wait_for 120 public_ok || fail "the canaries do not serve"
inv=$(grep -o 'invocation [0-9a-f]*' $S4C/4cca-remote.txt | cut -d' ' -f2)
...
ADMIT=79c5ecf24eb48a70e2bb20f4bca684b4d5e3c7700f9bf9d38735c19509898ce4 bash ~/enclave-bench/rs4-20260925/pkg/accept.sh > $S4C/4cca-accept.txt 2>&1 || fail "the relay slice's acceptance fails after the restart (4cca-accept.txt)"
say "4c-c-a APPLIED and checked: the allowlist is 04e953a4,10622d98,8ab7a159,${NEWM:0:12}; metal-iso0 re-attached on 8ab7a159; canaries serve; the predictor KAT PASS and ADMIT accept.sh ok"
