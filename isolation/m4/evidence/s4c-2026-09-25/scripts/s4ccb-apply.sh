...
fail() { set +e; trap '' TERM INT; say "4c-c-b CHECK FAILED: $* -> rolling back to dist-iso-b3109929 without the release opt-in"; FROM_APPLY="$TOKV" FROM_APPLY_WHY="4c-c-b at $T0: $*" "$S4C/s4ccb-rollback.sh"; local rc=$?; [ $rc = 0 ] && exit 20; say "ROLLBACK FAILED rc=$rc: ESCALATE to Codex (backup $CB4)"; exit 24; }
...
PY
T0=$(date -u '+%Y-%m-%d %H:%M:%S UTC'); say "4c-c-b: dist -> dist-iso-f6cbd75a + isolation.release=true; restarting enclave-metal-iso (the node CVM reboots)"
...
say "4c-c-b: the node ATTESTS ${NEWM:0:16} (raw report 0x90), overlay f6cbd75a"
...
# gsup read the opt-in from the host config and passed ISOLATION_RELEASE to the supervisor
grep -q 'attested release OPTED IN' $S4C/4ccb-node-journal.txt || fail "gsup did not report the attested-release opt-in"
