#!/usr/bin/env bash
# S5 TREE SWITCH to the FIXED release (enclave-87: the canaries relaunch on the release whose front keeps the app's bytes
# off the host console): guestd keeps its binary (guestd.4e78ba80, 0ee7ed91) and every flag, and ONLY -isolation moves
# from iso-aa6c985c (release 79c5ecf2) to iso-4cdd5169 (release 52156652, enclave-53's build, bf GO, installed inert by
# s5-install.sh). ONE guestd restart. Derived from the reviewed s4t-apply.sh. What differs from s4t, and why:
#   - the 3 canaries are RELEASE guests on 79c5ecf2 now (step 4e). The restart re-adopts them by their RECORDED
#     measurement (persist.go adoptOne), which s5-adoption-preflight.sh proves with the NEW tree's judge; they move to
#     52156652 only at each canary's owner restart afterwards (e4-restart.sh), one at a time;
#   - guestd's and the public checks use 4e's keys (lib-e4.sh check_guestd4 / public_ok4), not S0's;
...
grep -qE '^0::/.*/s5t-apply-[0-9]{8}T[0-9]{6}Z\.service$' /proc/self/cgroup || { say4 "REFUSING: run the tree switch detached, through s5t-run.sh"; exit 2; }
[ "$IMG" != "$OIMG" ] && [ "$T" != "$OT" ] || { say4 "REFUSING: lib5.sh names the live tree as the target"; exit 2; }
B=$PROD/bin/guestd.${BINC:0:8}; BAK=$EV/secret/enclave-guestd.service.bak-pre-5t; J=$S4/5t-guestd-journal.txt
...
adopt_fresh 7200 || { say4 "REFUSING: no adoption preflight for guestd $BINC / tree $IMG in the last 2 h (run s5-adoption-preflight.sh)"; exit 12; }
...
check_guestd4 || { say4 "REFUSING: guestd is not at 65536/1600 with the 3 canaries on their 4e keys"; exit 17; }
...
python3 - "$U" "$B" "$OT/isolation" "$LEG" "$FLOOR" <<'PY' || { say4 "REFUSING: ExecStart is not the live line (iso-aa6c985c)"; exit 18; }
import sys,shlex
...
"$S4/s5-adoption-preflight.sh" "$BINC" "$BSHA" || { say4 "REFUSING: the adoption preflight fails now"; exit 18; }
...
TOK=$EV/secret/5t-rollback-token; TOKV=$(head -c 16 /dev/urandom | od -An -tx1 | tr -d ' \n')
( umask 077; echo "$TOKV" > "$TOK" ) || { say4 "REFUSING: cannot write the rollback token"; exit 19; }
cp -p "$U" "$BAK"; chmod 600 "$BAK"
# FROM_APPLY: the preflight above verified the 3 canaries alone (on their 4e keys) and an empty non-canary set seconds
# before; the rollback skips its gate, which needs a READABLE guestd (the one thing a failed switch may not have)
fail() {
  set +e; trap '' TERM INT
  say4 "5T CHECK FAILED: $* -> rolling back to the iso-aa6c985c unit"
  FROM_APPLY="$TOKV" FROM_APPLY_WHY="5t apply at $T0: $*" "$S4/s5t-rollback.sh"; local rc=$?
...
python3 - "$U" "$OT/isolation" "$T/isolation" <<'PY' || { if cmp -s "$U" "$BAK"; then mv "$BAK" "$BAK.unused-$(date -u +%Y%m%dT%H%M%SZ)"; rm -f "$TOK"; say4 "5T: the unit edit failed and the unit is unchanged: nothing restarted"; exit 21; fi; fail "the unit edit"; }
import sys,os,shlex
...
PY
T0=$(date -u '+%Y-%m-%d %H:%M:%S UTC'); say4 "5T: daemon-reload + restart (guestd.${BINC:0:8}, -isolation ${OT##*/} -> ${T##*/}; all else unchanged)"
...
watch4d() {
  ...
  while :; do
    ...
    [ "$nr" = 0 ] || { say4 "5T: guestd restarted ($nr)"; return 2; }
    journalctl --user -u enclave-guestd.service --since "$T0" --no-pager -o cat > "$J" 2>/dev/null || true
    grep -qE "not adopted, ended|stopped [0-9]+ guest unit" "$J" && { say4 "5T: the new guestd dropped or stopped a guest"; return 2; }
...
wait_for 120 public_ok4 || fail "the canaries do not serve on their 4e keys"
...
say4 "5T APPLIED and checked (restart at $T0): -isolation is ${T##*/} (release ${REL:0:8}); the 3 canaries re-adopted on 79c5ecf2. Next: the 10-min gate, then each canary's owner restart onto 52156652; rollback = s5t-rollback.sh"
