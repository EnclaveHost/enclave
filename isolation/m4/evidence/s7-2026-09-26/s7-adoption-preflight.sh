#!/usr/bin/env bash
# S5 (the FIXED tree switch) ADOPTION preflight, derived from s4d-adoption-preflight.sh with lib7.sh: the NEW tree is
# iso-b63c2def and the adopted guests are the 3 canaries as RELEASE guests on 52156652 (since 4e). It changes nothing: it opens one attestation session per canary through its
