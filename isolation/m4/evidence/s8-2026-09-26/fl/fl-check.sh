#!/usr/bin/env bash
# One of Steven's apps' FIRST LAUNCH (on whichever admitted release guestd's tree built: f7888d86, 5db18199 after S8), checked READ-ONLY (started by fl-watch.sh at its setConfig). Never acts,
...
# must be that switch's release; one on anything else means the switch did not take. S8 (5db18199) is checked first: a
# guest created after it must be 5db18199; else after S7 it must be f7888d86. s8t-rollback.sh removes the S8 epoch.
...
for sw in "s8 5db18199ef0d321ea9dc8c81e385cb057efd05c2ef5d29e471b81fb2b78c2a77" "s7 f7888d8690845cbb862c1fbcae0a22f5458fcb891de7d0d3ae31ea927536b7ca"; do
  sn=${sw%% *}; srel=${sw##* }
...
WXC=-
if [ "$GREL" = 5db18199ef0d321ea9dc8c81e385cb057efd05c2ef5d29e471b81fb2b78c2a77 ]; then
