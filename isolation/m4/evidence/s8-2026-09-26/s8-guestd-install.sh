#!/usr/bin/env bash
# S8 1c: the NEW guestd binary (the per-release W^X naming: -isolation-release, -legacy-isolation-release,
# -unrecorded-releases; isolation/wx-at-attest @ 0c087de8, bf+5d GO) under its own name, INERT (nothing references it
# until s8t-apply.sh). Derived from s4-guestd-install.sh (4d's 1c). Built twice in a clean worktree at 0c087de8, the second
# with an EMPTY Go cache; the two must agree and equal the sha a reviewer reproduced independently (2nd argument). It must
# carry the host floor (FLOORC) and state every flag the S8 unit passes.
# Usage: s8-guestd-install.sh <0c087de8 full commit> <the independently reproduced sha256>
...
[ "$BINC" = "$NBINC" ] || { say4 "REFUSING: S8's binary is built from $NBINC"; exit 3; }
...
u=$("$W/g1" -h 2>&1 || true)
for f in -release -legacy-isolation -instance-prefix -guest-mem-mib -guest-cpus -guest-host-floor-mib -ticket-port -isolation-release -legacy-isolation-release -unrecorded-releases; do
