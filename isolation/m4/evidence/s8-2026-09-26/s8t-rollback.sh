...
rm -f ~/enclave-bench/fl-20260926/state/s8-switched-epoch   # back on iso-b63c2def: new guests are f7888d86 again (the first-launch check)
...
X=$(systemctl --user show enclave-guestd.service -p ExecStart --value) || { say4 "ROLLBACK CHECK FAILED: ExecStart unreadable"; exit 21; }
grep -Fq -- "path=$B4 ;" <<<"$X" && grep -Fq -- " -isolation $OT/isolation " <<<"$X" && ! grep -qE -- '-isolation-release|-unrecorded-releases' <<<"$X" \
  || { say4 "ROLLBACK CHECK FAILED: the live ExecStart is not guestd.4e78ba80 on iso-b63c2def without per-release flags"; exit 21; }
