# Inventory: domain release 4cd26e58 (release id aee2059ffcc7bd8a459001cba02a7e9139d6a4aa964fd6de68047407f8597532)

**The seccomp-statement release.** Source: branch **isolation/seccomp-evidence**, commit
**4cd26e58bf44958fdf37b063408640f35596dc02** (not main; enclave-87's ruling). enclave-5d and enclave-bf gave GO on the
chain through 47b7b520, and enclave-87 approved it; 4cd26e58 adds only a host-side table change (f7888d86 leaves
judge.mjs's LEGACY_WX_RELEASES), which is not in the release bytes. Over 5db18199 (0c087de8) it carries:
- init STATES the app runtime's seccomp filter once it is installed: the sha256 of the exact BPF program
  (m2/app-seccomp.h, 71 instructions, d4d17c9f53832439c92a3232fd09feed8b28f0e3c7dd357d26468a9566f62b66), handed to the
  front over a bounded read (10 s, fail closed: an app that gives no statement is killed, not started);
- the front's attester checks at each attestation that every runtime process is under a filter and carries
  seccomp=<hash> in the runtime self-test;
- host-side changes of the same chain (the judge's per-release seccomp table, guestd naming every tree), which this
  image does not contain.
The filter itself is unchanged from 5db18199: its exact 568 bytes occur in both releases' init. It replaces 5db18199
for the next tree switch, and 5db18199 is its rollback.

**Only Enclave's own template/front and template/init changed from 5db18199.** `check-third-party.py` against 5db18199
lists those two as the only differing files. enclave-bf reproduced the release independently (its own worktree at
4cd26e58, its own musl from build-musl.sh with libc.a 4f72e098…, domain-release.sh with a cold Go cache): the same id,
and `diff -r` against this release is identical; the init is static and the d4d17c9f filter (568 bytes) occurs once in
it (enclave-bf's GO).

This is a DIFF NOTE against release 0181bce3, as 79c5ecf2's, 52156652's, f7888d86's and 5db18199's were. The
third-party parts of that release's inventory hold here unchanged, because the bytes are the same, and they are
...
|---|---|---|
| template/init | b8aa8dcb9f17e7abd2d73ab9dcca946d1124e7f44d8b234d99e13792f4026dda | `/usr/bin/gcc -specs <musl prefix>/lib/musl-gcc.specs -static -O2 -o init isolation/m2/dominit.c`, gcc 16.2.1, musl 1.2.6 built by isolation/m2/build-musl.sh |
| template/front | b4f56fedf5389dbb452cf9677fcc57f6916d7b1ae5b70d2e02b77facdd6c9701 | `GOFLAGS= CGO_ENABLED=0 go build -trimpath -buildvcs=false -ldflags='-s -w -buildid=' ./front` (the PRODUCTION front: no releaselab tag), go1.27.0 (Arch go 2:1.27.0-1, toolchain default GOEXPERIMENT nodwarf5) |

**template/init: musl, not glibc; NEW bytes (the seccomp statement).** isolation/m2/dominit.c, app-seccomp.h and the
new sha256-min.h changed from 0c087de8 to 4cd26e58. It was built twice by cut-release.sh, against a musl prefix made
by 4cd26e58's build-musl.sh (libc.a sha256 4f72e098…, 5db18199's; the source's sha256 and musl's release signature
both checked), and is b8aa8dcb… (102664 bytes). sha256-min.h is in-repository: a minimal FIPS 180-4 SHA-256 (the round
constants are the standard's) for the one digest init states; its header names no other origin, and it is checked
against an independent implementation by m2/test-seccomp-statement.sh.
Rebuilt here once more with `-Wl,-Map` by app-image-template.sh's exact command (the three sources from 4cd26e58, the
same musl prefix), it is the same bytes, b8aa8dcb…, so the map describes the released init:
- **The link loads:**
  - musl's Scrt1.o, crti.o and crtn.o, and 147 members of musl's libc.a, counted from the map's "Archive member
    included" section. That is 5db18199's 142 (re-derived the same way: 00355508…) plus kill.lo, poll.lo, rename.lo,
    sscanf.lo and vsscanf.lo, which serve the statement: poll bounds the read, kill stops an app that gives none,
    sscanf checks its form, and rename records it for the front atomically. prctl.lo, setgroups.lo and syscall.lo
    serve the privilege drop, the holds and the seccomp filter (installed through prctl/syscall); capset and capget go
    through syscall().
...
**template/front:** 5db18199's front plus the seccomp check at each attestation: every runtime process under a filter,
and seccomp=<hash> in the self-test (isolation/m2/front/runtime.go, main.go; in-repository).
...
## Findings
- **Nothing third-party changed from 5db18199.** Only Enclave's own front and init changed.
