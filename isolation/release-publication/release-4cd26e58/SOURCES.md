# Corresponding source: domain release 4cd26e58 (release id aee2059f…, the seccomp-statement release)

Release 4cd26e58's third-party files are byte-identical to release 5db18199's, and so to 79c5ecf2's
(check-third-party.py): only Enclave's own template/front and template/init changed. So this is 79c5ecf2's list
unchanged, and the corresponding source is 79c5ecf2's bundle. As there, the third-party files equal release 0181bce3's,
except that template/init links musl instead of glibc; musl's source is in the same bundle. For each component that the
release ships as a binary, this lists the source that builds those bytes, and the recipe that built them.
...
- **The corresponding source goes out as ONE bundle, distributed next to the release tarball from the same place:**
  `enclave-guest-corresponding-source-11b366ea5a41.tar`, sha256
  21752b26d36f33198ffc8aadf3b8fdff235646dd35eaa694eb6d5473f6d093da, 328878080 bytes. It is not merely pointed to:
  ...
  - It holds every file below, SHA256SUMS, and a README naming the releases it covers: 5c3561f9, a4f22748 and
    79c5ecf2. All three have the same third-party bytes, except that 79c5ecf2's init links musl.
  - **It serves this release unchanged.** aee2059f has 79c5ecf2's third-party bytes exactly (as 52156652, f7888d86
    and 5db18199 do), and the README does not name any of them. A bundle whose README names more releases would be a
    different tar under the same file name, so it is not made here: at most one such tar may ever be published.
...
- **The release tarball itself carries template/init's source:** source/isolation/m2/dominit.c with the local
  headers it includes (app-seccomp.h, sha256-min.h), so it compiles as shipped. init links musl (MIT) statically. No
  LGPL code is statically linked into this release. glibc is shipped only as the shared runtime set in template/rt/
  that wasmtime is dynamically linked with, with its complete source in the bundle.
...
|---|---|---|---|
| arch-grub-2.14-1-reverts.patch | a7172f6c89df32ceffecf2e3e753c938ee6ebb942a010b68f89499a020750482 | 2079 | the two commits Arch's prepare() reverts (1a5417f3, ac042f3f), as their result on grub-2.14: a history-less tree cannot replay `git revert` |
...
| arch-packaging-grub-2-2.14-1.tar.xz | 1af0f26545a55cd1e4f80c9ffc05377be093ed12e0ee1e17fef0a514d46af121 | 28248 | the recipe and its three patches at tag 2-2.14-1 (984cb119) |
...
| firmware-build.env | 552632cf5f958c138d75fe6fa1ae289562abcb607703f7d5ea0cc30985b434d6 | 73 | the firmware's recorded build-time inputs: the memdisk's volume ID and time (TZ=UTC0), and the edk2 path length |
...
| musl-1.2.6.tar.gz | d585fd3b613c66151fc3249e8ed44f77020cb5e6c1e635a616d3f9f82460512a | 1082499 | musl 1.2.6 (musl.libc.org), no patches: template/init's libc from image commit aa6c985c on |
| musl-1.2.6.tar.gz.asc | 8a696228404cd4bae6379cf7538fc5d18c133b0ada54a41c1c6cef31d3c22ab6 | 490 | its signature, by musl's release key 836489290BB6B70F99FFDA0556BCDB593020450F |
| rust-1.98.0-COPYRIGHT | 172020dbfd5b53a226dfde77616190a48dcff519b0bc0e6deb91a8450782c4af | 1571 | the Rust 1.98.0 standard library's notice (rust-lang/rust 88d9e12a; wasmtime's std) |
| rust-1.98.0-LICENSE-APACHE | 62c7a1e35f56406896d7aa7ca52d0cc0d272ac022b5d2796e7d6905db8a3636a | 9723 | its Apache-2.0 text |
| rust-1.98.0-LICENSE-MIT | b71bd43a069ca0641a9ecfe585ca7b3c53b5cc1608f8b68321168698e28b5ea1 | 1068 | its MIT text |
| rust-src-1.98.1.tar.xz | 5c846ebcebcc7e2e0777a4cdaa12051691593f16a7e94edbae5e6241cc62d98c | 5741168 | Rust 1.98.1's library source (static.rust-lang.org; tag 1.98.1 = 48a229ce): the kernel's CONFIG_RUST builds core from it |
| unifont-17.0.03.bdf.gz | 30d50302daca631e2cf454d9747117d98c9f35c0b18af4f67d2fa5e116992033 | 1259456 | the font source Arch's GRUB build has (b2sum b824e469... as the PKGBUILD pins); GRUB generates widthspec.h from it, compiled into the normal module |
| unifont-17.0.03.bdf.gz.sig | ed5dc6f12accead3739145e0baa95a0547e8d8f454fd45fba915cd1d372f56fe | 566 | its signature |
...
- **The kernel and modules:**
  ...
  - The kernel has CONFIG_RUST=y, and its rust/Makefile builds core from Rust 1.98.1's library source, which lives
    outside the kernel tree: rust-src-1.98.1.tar.xz, in the bundle.
- **glibc and GCC:** the same way, from their recipes. The recipes name the commits above.
- **GRUB:** the same way, with:
  - gnulib at the revision above (GRUB's bootstrap checks it out);
  - unifont-17.0.03.bdf.gz as the font source;
  - arch-grub-2.14-1-reverts.patch applied in place of prepare()'s two `git revert`s.
- **musl and template/init:**
  ...
  - This repository's isolation/m2/build-musl.sh does exactly that, with the tarball's sha256 and musl's release
    ...
    gives the same release id. This release's init links the same musl (libc.a 4f72e098…, built by 4cd26e58's
    build-musl.sh) with 4cd26e58's dominit.c, app-seccomp.h and sha256-min.h: b8aa8dcb…, 147 libc.a members
    (INVENTORY.md).
...
4cd26e58bf44958fdf37b063408640f35596dc02 on branch isolation/seccomp-evidence (INVENTORY.md gives the commands). The
repository LICENSE covers them.
