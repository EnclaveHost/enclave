# Corresponding source: domain release 4cdd5169 (release id 52156652…, the console-guard release)

Release 4cdd5169's third-party files are byte-identical to release 79c5ecf2's (check-third-party.py): only Enclave's own
template/front changed. So this is 79c5ecf2's list unchanged, and the corresponding source is 79c5ecf2's bundle. As
there, the third-party files equal release 0181bce3's, except that template/init links musl instead of glibc; musl's
source is in the same bundle. For each component that the
release ships as a binary, this lists the source that builds those bytes, and the recipe
that built them.
...
- **The corresponding source goes out as ONE bundle, distributed next to the release tarball from the same place:**
  ...
  - **It serves this release unchanged.** 52156652 has 79c5ecf2's third-party bytes exactly, and the README does not
    name 52156652. A bundle whose README names four releases would be a different tar under the same file name, so
    it is not made here: at most one of the two may ever be published.
...
- **musl and template/init:**
  ...
  - This repository's isolation/m2/build-musl.sh does exactly that, with the tarball's sha256 and musl's release
    ...
    gives the same release id. This release's init is those same bytes (ba7f7ff0…), built from 4cdd5169's
    build-musl.sh.
...
4cdd516924981eeacefc8ccaefb4e7fc0e1ec939 (INVENTORY.md gives the commands). The repository LICENSE covers them.
