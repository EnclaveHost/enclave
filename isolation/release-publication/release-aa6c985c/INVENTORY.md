# Inventory: domain release aa6c985c (release id 79c5ecf24eb48a70e2bb20f4bca684b4d5e3c7700f9bf9d38735c19509898ce4)

**The production release.** It was built at image commit aa6c985c688e73ffc2ec547b691c2d804e50ad69, reviewed, and
reproduced from scratch by its reviewers. It replaces a4f22748 (17e182a8) for publication: template/init now links musl
(MIT) statically instead of glibc. The rollback is 5c3561f9 (0181bce3).

This is a DIFF NOTE against release 0181bce3. The third-party parts of that release's inventory hold here unchanged,
because the bytes are the same, and they are reproduced at the end.
...
| template/front | bd066066d573b9978af3ac7ed456deae9c6e8392976c2a473530979be8e87bd1 | `GOFLAGS= CGO_ENABLED=0 go build -trimpath -buildvcs=false -ldflags='-s -w -buildid=' ./front` (the PRODUCTION front: no releaselab tag), go1.27.0 (Arch go 2:1.27.0-1, toolchain default GOEXPERIMENT nodwarf5) |

**template/init: musl, not glibc.** Rebuilt here from `git show aa6c985c:isolation/m2/dominit.c`, against a musl prefix
this recipe built with that commit's build-musl.sh (libc.a sha256 4f72e098…, the source's sha256 and musl's release
signature both checked), it is byte-identical: ba7f7ff0….
...
**template/front:** as in 17e182a8's release.
...
## Findings
- **Nothing third-party changed except the libc in init,** now musl, MIT.
