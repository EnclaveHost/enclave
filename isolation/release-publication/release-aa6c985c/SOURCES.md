# Corresponding source: domain release aa6c985c (release id 79c5ecf2…, the production release)

Release aa6c985c's third-party files are byte-identical to release 0181bce3's (check-third-party.py), except that
template/init now links musl instead of glibc; musl's source is in the same bundle. For each component that the
...
- **The corresponding source goes out as ONE bundle, distributed next to the release tarball from the same place:**
  ...
  - It holds every file below, SHA256SUMS, and a README naming the releases it covers: 5c3561f9,
    a4f22748 and 79c5ecf2 (this one). All three have the same third-party bytes except this release's init, which
    links musl.
...
aa6c985c688e73ffc2ec547b691c2d804e50ad69 (INVENTORY.md gives the commands). The repository LICENSE covers them.
