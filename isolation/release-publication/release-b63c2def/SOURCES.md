# Corresponding source: domain release b63c2def (release id f7888d86…, the hardened release)

Release b63c2def's third-party files are byte-identical to release 52156652's, and so to 79c5ecf2's
...
- **The corresponding source goes out as ONE bundle, distributed next to the release tarball from the same place:**
  ...
  - **It serves this release unchanged.** f7888d86 has 79c5ecf2's third-party bytes exactly (as 52156652 does), and
    the README does not name either of them. A bundle whose README names more releases would be a different tar under
    the same file name, so it is not made here: at most one such tar may ever be published.
...
- **musl and template/init:**
  ...
  - This repository's isolation/m2/build-musl.sh does exactly that, with the tarball's sha256 and musl's release
    ...
    gives the same release id. This release's init links the same musl (libc.a 4f72e098…, built by b63c2def's
    build-musl.sh) with b63c2def's hardened dominit.c: 52640baf…, 142 libc.a members (INVENTORY.md).
...
b63c2def6e9b12088c30594b8cb58c7273903a60 (INVENTORY.md gives the commands). The repository LICENSE covers them.
