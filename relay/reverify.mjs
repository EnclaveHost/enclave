...
export function createReverifier({ mode = "shadow", bundle = null, repo = "EnclaveHost/enclave", minTcb = undefined, cacheDir = null, expectationsTtlMs = 60 * 60 * 1000,
                                   timeoutMs = 20000, releaseIndex = null, verify = null, expectationsFor = null, log = () => {}, now = () => new Date() } = {}) {
  mode = modeOf(mode);
  const byEndpoint = new Map();          // endpoint -> brief verdict
  let expectations = null, expectationsAt = 0, loading = null, collateral = null, running = false;
  ...
  async function refreshExpectations() {
    ...
    const B = await load();
    const e = expectationsFor ? await expectationsFor() : await B.releaseExpectations({ repo, timeoutMs, ...(releaseIndex ? { apiBase: releaseIndex.apiBase, downloadBase: releaseIndex.downloadBase ?? releaseIndex.apiBase } : {}) });
  ...
  return { mode, run, annotate, eligible, ineligibleReason, verdictOf: (endpoint) => byEndpoint.get(endpoint) ?? null, stats: () => ({ ...stats, expectationsAt: expectationsAt ? new Date(expectationsAt).toISOString() : null, expectations: expectations ? { ok: expectations.ok, latestTag: expectations.latestTag ?? null, allowed: expectations.allowed.map((a) => a.tag) } : null }) };
}
