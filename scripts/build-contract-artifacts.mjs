...
function compile(name, viaIR, fileBase, alsoAbi, runs) {
...
// - the creation transaction reverts, on mainnet, after the gas is spent. The
// size printed above is the CREATION bytecode, which is always larger, so it
// tells you nothing about the limit. EnclaveDeployments has lived within a few
// dozen bytes of the wall since rev 7 and is the reason this check exists: the
// failure it prevents is a governance deploy that burns gas and lands nothing.
const EIP170 = 24576;
...
for (const def of DEFS) {
  ...
  if (free < 0) throw new Error(
    `${def.name} runtime is ${runtime} bytes, ${-free} OVER the EIP-170 limit (${EIP170}) - it cannot be deployed. ` +
    `Shrink it, lower its optimizer runs (see the DEFS entry), or move code behind a companion contract.`);
  console.log(`  ${def.name.padEnd(21)} ${(bytecode.length / 2 - 1).toString().padStart(6)} bytes · ${Object.keys(sel).length} fns${def.viaIR ? " · viaIR" : ""}`
    + (free < 1024 ? `  \x1b[33m[EIP-170: ${free} bytes free]\x1b[0m` : ""));
}
