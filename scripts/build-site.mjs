   ...
   concern and is left untouched. */
{
  ...
  fs.writeFileSync(path.join(DIST, "csp-script-src.txt"),
    ...
    "# script-src for the enclave.host + nan.host Caddy vhosts: paste this in and\n" +
    "# DROP 'unsafe-inline' and https://esm.sh from the live directive. The hashes\n" +
    "# cover every executable inline <script> in dist/ and change when those scripts\n" +
    "# change, so re-apply on each deploy (or have the deploy read this file).\n" +
