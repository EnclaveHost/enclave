...
//                         APP_CATALOG_ADDRESS already has code (orphans the live
//                         catalog + its approvals). Refused by default.
...
async function main() {
  ...
  // Guard: on mainnet, refuse to orphan a live catalog. If the config already
  // names a catalog that HAS CODE on this chain, a fresh deploy would mint a new
  // one and rewrite every config to it — silently stranding published apps and
  // approvals on the old address. First deploy / testnet are unaffected.
  if (isMainnet && !REPLACE) {
    const m = fs.readFileSync(CONFIG, "utf8").match(/-\s*APP_CATALOG_ADDRESS:\s*"(0x[0-9a-fA-F]{40})"/);
    if (m) {
      const existing = getAddress(m[1]);
      const code = await pub.getCode({ address: existing }).catch(() => null);
      if (code && code !== "0x")
        die(`APP_CATALOG_ADDRESS already points at ${existing}, which HAS CODE on ${netName}.\n`
          + `  Re-deploying mints a NEW catalog and rewrites every config to it, orphaning the live one\n`
          + `  (published apps + approvals stay on the old address). Pass --replace to proceed, or --dry-run to inspect.`);
    }
  }
