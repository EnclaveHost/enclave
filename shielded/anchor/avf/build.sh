...
case "$NAME" in
  ...
  engine-pvm)  # the engine FOR THE VM: the shielded module with the fd-adopting hook, and libengine.so
           ...
           "$CXX" -O2 -std=c++17 -fPIC -march=armv8.2-a+dotprod -DGGML_MAX_NAME=128 "${INC[@]}" -I"$GG" -I"$HERE/payload" -shared -o "$E/libengine.so" "$HERE/payload/engine.cpp" "$E/mtp.o" "$E/pads.o" "$E/bank.o" "$E/http.o" "$E/prefixkv.o" "$E/nacl.o" "$E/poly.o" -L"$GA/lib" -lllama -lggml -lggml-base -llog -ldl -Wl,-soname,libengine.so
  ...
  anchor)       # the anchor + the harness's worker client over an fd (wire-fd.c wraps the shipped shielded-wire.c).
                ...
                SRCS=("$HERE/payload/anchor_payload.c" "$HERE/payload/anchor_pins.c" "$HERE/payload/anchor_names.c" "$HERE/payload/anchor_gguf.c" "$CORE/anchor-core.c" "$GG/shielded-simd.c" "$GG/shielded-field.c"
