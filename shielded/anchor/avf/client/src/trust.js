...
export const CLIENT_VERSION = "0.4.1";   // inside the artifact's bytes: it cannot be claimed without changing the artifact's hash
...
export async function verifyPolicy(env, { state, now = Date.now(), clientVersion = CLIENT_VERSION } = {}) {
  ...
  if (!list(b.formats, /./, 2) || !b.formats.every((f) => f === PVM_APP_EVIDENCE_FORMAT_V2 || f === PVM_APP_EVIDENCE_FORMAT)) return no("the policy's evidence formats are not known to this client");
