...
set -uo pipefail
die() { echo "REFUSING: $*" >&2; exit 2; }   # defined FIRST: it was once called before it existed
...
# die is DEFINED BELOW, and calling it from here printed "die: command not found" and CARRIED ON --
...
# gate=per-row is part of the settings, and therefore of every key: a batch-gated result must not be
# servable as a per-row-gated one. One gate before 24 rows says nothing about rows 2..24.
SETTINGS="sampler=$SAMPLER${TEMPERATURE:+ temp=$TEMPERATURE} max_new=runner-default backend=npu nocool=${NOCOOL:-0} gate=per-row"
{
...
for line in "${PLIST[@]}"; do
  ...
  echo "[$id] $p"
  # EVERY measured row is gated, not just the first. The phone heats up as the batch runs, so a single
  # gate before row 01 establishes nothing about row 24.
  cool_gate || { echo "  FAILED: the phone is not in a comparable thermal state"; failed=$((failed+1)); row hot; continue; }
