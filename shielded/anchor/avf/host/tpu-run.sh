...
$ADB logcat -d -s anchor-host:I | sed 's/.*anchor-host: //' | grep -E "HOST FAIL|VM (error|stopped)|^TPU|LOCAL (turn [0-9]+ (A:|STATS)|done|failed)|VSOCK LOCAL (tpu|refused)" | cut -c1-${WIDTH:-460}
