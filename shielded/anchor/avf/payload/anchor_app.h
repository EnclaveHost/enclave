 ...
 *   APP bytes=<1..2^30> sha256=<64 lowercase hex>[ args=<even-length lowercase hex, 1..8192 bytes decoded>][ graph=<name>][ serve=http]
 ...
 * app takes its input from requests). The component itself arrives on
...
typedef struct { uint64_t bytes; uint8_t sha256[32]; char args[8193]; size_t args_len; char graph[65]; int http; } anchor_app_plan;
...
static inline int anchor_app_parse(const char *line, anchor_app_plan *plan) {
    ...
    int http = 0;
    if (!strncmp(p, " serve=http", 11)) { if (n) return 0; http = 1; p += 11; }
    if (*p != 0) return 0;
    plan->http = http;
