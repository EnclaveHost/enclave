 ...
 *   LOCAL model_bytes=<1..2^40> threads=<1..16> ctx=<512..32768>
 * Strict: exactly these three keys in this order, canonical decimal, single spaces, nothing after. Pure. */
#ifndef ANCHOR_LOCAL_H
#define ANCHOR_LOCAL_H
#include <stdint.h>
#include <string.h>
typedef struct { uint64_t model_bytes; int threads, ctx; } anchor_local_plan;
...
static inline int anchor_local_parse(const char *line, anchor_local_plan *plan) {
    ...
    if (!anchor_local_u64(&p, "ctx=", 512, 32768, &c) || *p != 0) return 0;
    plan->model_bytes = b; plan->threads = (int)t; plan->ctx = (int)c; return 1;
}
#endif
