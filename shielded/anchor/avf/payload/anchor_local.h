 ...
 * purpose: the target verifies every proposal) and how many tokens it proposes per step. Pure. */
#ifndef ANCHOR_LOCAL_H
#define ANCHOR_LOCAL_H
#include <stdint.h>
#include <string.h>
typedef struct { uint64_t model_bytes; int threads, ctx; uint64_t tpu_bundle_bytes; int bank, refill; uint64_t draft_bytes; int draft_max; } anchor_local_plan;
