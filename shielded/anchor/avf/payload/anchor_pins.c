...
typedef struct { uint32_t h[8]; uint8_t buf[64]; size_t used; uint64_t total; } sha256_ctx;
static void sha_init(sha256_ctx *c) { static const uint32_t iv[8] = { 0x6a09e667,0xbb67ae85,0x3c6ef372,0xa54ff53a,0x510e527f,0x9b05688c,0x1f83d9ab,0x5be0cd19 }; memcpy(c->h, iv, sizeof iv); c->used = 0; c->total = 0; }
...
static void sha_update(sha256_ctx *c, const uint8_t *m, size_t n) {
    c->total += n;
    while (n) { size_t take = 64 - c->used < n ? 64 - c->used : n; memcpy(c->buf + c->used, m, take); c->used += take; m += take; n -= take;
                if (c->used == 64) { sha_block(c->h, c->buf); c->used = 0; } }
}
