    ...
    for (;;) {
        ...
        if (spec) {   /* ---- a speculative turn: prefill all but the last prompt token, then draft -> verify rows -> accept ---- */
            ...
            if (tpu_stats) { ggml_backend_tpu_stats_t ts; tpu_stats(&ts, 1); const double ex = ts.exchanges ? (double)ts.exchanges : 1.0;
                ...
                outf("LOCAL tpu turn %d: ms per exchange, split: of unmask, verification dots %.3f | after it, releasing the pads %.3f | between exchanges (the VM's own graph work) %.3f | of wait, joining the correction after the reply %.3f | out-of-lane entries per exchange %.1f",
                     served, ts.ver_us / ex / 1e3, ts.free_us / ex / 1e3, ts.gap_us / ex / 1e3, ts.corr_join_us / ex / 1e3, ts.outlier_entries / ex); }
            continue;
        }
