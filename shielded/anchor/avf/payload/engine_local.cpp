    ...
    for (;;) {
        ...
        if (spec) {   /* ---- a speculative turn: prefill all but the last prompt token, then draft -> verify rows -> accept ---- */
            ...
            if (tpu_stats) { ggml_backend_tpu_stats_t ts; tpu_stats(&ts, 1); const double ex = ts.exchanges ? (double)ts.exchanges : 1.0;
                outf("LOCAL tpu turn %d: exchanges=%llu (%.1f/step, %.2f rows each) ms per exchange: mask %.3f link %.3f (corr %.3f mint %.3f wait %.3f) unmask %.3f | pads inline %llu refilled %llu bank_min %llu | verify n=%llu bad=%llu max=%llu rms=%.3f rails(-32768/-32767/+32767)=%llu/%llu/%llu | outliers kept %llu saturated %llu (CLIPPED %llu [hi %llu lo %llu] %s, max excess %llu, max err %.1f LSB)",
                     served, (unsigned long long)ts.exchanges, ex / (steps ? steps : 1), ts.rows / ex, ts.mask_us / ex / 1e3, ts.link_us / ex / 1e3, ts.corr_us / ex / 1e3, ts.window_mint_us / ex / 1e3, ts.wait_us / ex / 1e3, ts.unmask_us / ex / 1e3, (unsigned long long)ts.pads_minted_inline, (unsigned long long)ts.pads_refilled, (unsigned long long)ts.bank_min, (unsigned long long)ts.ver_n, (unsigned long long)ts.ver_bad, (unsigned long long)ts.ver_max, ts.ver_n ? sqrt(ts.ver_sq / (double)ts.ver_n) : 0.0, (unsigned long long)ts.rail_m32768, (unsigned long long)ts.rail_m32767, (unsigned long long)ts.rail_p32767, (unsigned long long)ts.outlier_entries, (unsigned long long)ts.saturated, (unsigned long long)ts.sat_clipped, (unsigned long long)ts.sat_hi, (unsigned long long)ts.sat_lo, ts.sat_repaired ? "REPAIRED" : "USED-AS-IS", (unsigned long long)ts.sat_max_excess, ts.sat_max_err_lsb); }
            continue;
        }
        ...
        if (tpu_stats) { ggml_backend_tpu_stats_t ts; tpu_stats(&ts, 1); const double ex = ts.exchanges ? (double)ts.exchanges : 1.0;
            outf("LOCAL tpu turn %d: exchanges=%llu (%.1f/token) ms per exchange: mask %.3f link %.3f (corr %.3f mint %.3f wait %.3f) unmask %.3f | rx/exch %.2f | KB/token out %.0f in %.0f | pads inline %llu (%.1f ms each) bank_min %llu | verify n=%llu bad=%llu max=%llu rms=%.3f rails(-32768/-32767/+32767)=%llu/%llu/%llu | outliers kept %llu saturated %llu (CLIPPED %llu [hi %llu lo %llu] %s, max excess %llu, max err %.1f LSB) redrawn %llu | %s",
                 ...
                 ts.pads_minted_inline ? ts.mint_inline_us / 1e3 / ts.pads_minted_inline : 0.0, (unsigned long long)ts.bank_min, (unsigned long long)ts.ver_n, (unsigned long long)ts.ver_bad, (unsigned long long)ts.ver_max, ts.ver_n ? sqrt(ts.ver_sq / (double)ts.ver_n) : 0.0, (unsigned long long)ts.rail_m32768, (unsigned long long)ts.rail_m32767, (unsigned long long)ts.rail_p32767, (unsigned long long)ts.outlier_entries, (unsigned long long)ts.saturated, (unsigned long long)ts.sat_clipped, (unsigned long long)ts.sat_hi, (unsigned long long)ts.sat_lo, ts.sat_repaired ? "REPAIRED" : "USED-AS-IS", (unsigned long long)ts.sat_max_excess, ts.sat_max_err_lsb, (unsigned long long)ts.pads_redrawn, mem_line().c_str()); }
    }
