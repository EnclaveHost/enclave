...
//   node shielded/anchor/avf/runner/proof-agent-cli.mjs --config <agent.json> --state <dir> [--once | --ticks N]
...
if (!cfgPath || !stateDir) die("usage: proof-agent-cli.mjs --config <agent.json> --state <dir> [--once | --ticks N]");
...
try { config = JSON.parse(fs.readFileSync(cfgPath, "utf8")); checked = checkAgentConfig(config); } catch (e) { die(e.message); }
...
let agent;
try { agent = await createProofAgent({ config, publicClient, account, stateDir, log }); } catch (e) { die(e.message); }
