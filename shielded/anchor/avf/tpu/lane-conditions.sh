...
# lane-quality.sh the driver and its gate are frozen into <out_dir> and only that copy runs; RUNS.tsv records each run's
# exit status, never assumed.
set -uo pipefail
...
cp "$H/lane-run2.sh" "$OUTD/driver/tpu/lane-run2.sh" && cp "$H/../host/coolgate.sh" "$OUTD/driver/host/coolgate.sh" && cp "$CONDS" "$OUTD/conditions.tsv" || exit 2
...
while IFS=$'\t' read -r label cond extra; do
  ...
  EXTRA="$extra" OUT="$OUTD" "$DRV" "$label" > "$OUTD/$label.driver" 2>&1; rc=$?
