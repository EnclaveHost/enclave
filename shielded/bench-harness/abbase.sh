...
# foreign CPU sampled during each run (run9 .intr), huge pages (.thp), placement (.threads)
...
sha256sum $CPU_SO $M/llama-conv/bin/libggml-base.so $BENCH /home/steven/Projects/enclave/wasm/ggml-shielded/libggml-shielded.so > $B/base.ids
for i in 1 2 3 4 5 6; do bash $B/waitfree.sh; bash $B/waitquiet2.sh
  echo "bs-$i pre: $(grep -E '^(MemFree|AnonHugePages):' /proc/meminfo | tr -s ' ' | tr '\n' ' ') now:[$(python3 $B/cpunow.py 3 10)]"
  WORKERS_OVERRIDE="$FIT" $B/run9.sh bs-$i 1 8 64; echo "  plain-hash $(grep -h 'plain tokens' $B/bs-$i.err | md5sum | cut -c1-12) thp: $(tail -1 $B/bs-$i.thp)"; done
echo QUEUE-DONE
