#!/bin/bash
# register-row GATED_DELTA_NET: ENCLAVE_GGML_GDN_REGROW=0 vs =1, same binary and
...
sha256sum $CPU_SO $M/llama-conv/bin/libggml-base.so $B/bench-spec2 /home/steven/Projects/enclave/wasm/ggml-shielded/libggml-shielded.so > $B/rg.ids
run() { bash $B/waitfree.sh; bash $B/waitquiet.sh; v=1; [ "$2" = off ] && v=0
  WORKERS_OVERRIDE="$FIT" ENCLAVE_GGML_CONV_INPLACE=1 ENCLAVE_GGML_GDN_REGROW=$v $B/run7.sh rg-$2-$1 1 8 64; }
