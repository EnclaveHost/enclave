#!/bin/bash
# op-level check of the register-row GATED_DELTA_NET: the [opprof-rows] 2-row
...
run() { bash $B/waitfree.sh; bash $B/waitquiet.sh; v=1; [ "$2" = off ] && v=0
  WORKERS_OVERRIDE="$FIT" ENCLAVE_OP_PROFILE=1 ENCLAVE_GGML_CONV_INPLACE=1 ENCLAVE_GGML_GDN_REGROW=$v $B/run7.sh rp-$2-$1 1 8 64; }
