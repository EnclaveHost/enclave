...
int main(int argc, char **argv) {
    ...
    /* ---- 0. warm-up (WARM=1): open the shielded link, capture the m=1 graphs
     * and, with PREFILL_REPS=n, time n further prefills of the same prompt so
     * the prefill cost is measured with the link up and the graphs captured,
     * not once with the whole weight upload inside it. ---- */
    double prefill_warm_ms = 0; int prefill_reps = env_int("PREFILL_REPS", 0);
    if (env_int("WARM", 0)) {
        if (ell_decode_seq_full(ctx, model, seq, 0, toks.data(), n, plog.data())) { fprintf(stderr, "warm prefill failed\n"); return 2; }
        int t = argmax(plog.data() + (size_t)(n - 1) * nv);
        for (int i = 0; i < 3; i++) {
            if (ell_decode_seq_full(ctx, model, seq, n + i, &t, 1, logits.data())) { fprintf(stderr, "warm decode failed\n"); return 2; }
            t = argmax(logits.data());
        }
        ell_seq_remove(ctx, seq);
        std::vector<double> pf;
        for (int r = 0; r < prefill_reps; r++) {
            const int64_t a = ggml_time_us();
            if (ell_decode_seq_full(ctx, model, seq, 0, toks.data(), n, plog.data())) { fprintf(stderr, "timed prefill failed\n"); return 2; }
            pf.push_back((ggml_time_us() - a) / 1e3);
            ell_seq_remove(ctx, seq);
        }
        if (!pf.empty()) { std::sort(pf.begin(), pf.end()); prefill_warm_ms = pf[pf.size() / 2]; }
        fprintf(stderr, "[bench] warm: link opened; %d timed prefills, median %.1f ms\n", prefill_reps, prefill_warm_ms);
    }
    ...
    {   /* the greedy token ids, so two builds can be compared exactly */
        std::string ids;
        for (size_t i = 0; i < plain.size(); i++) { if (i) ids += ','; ids += std::to_string(plain[i]); }
        fprintf(stderr, "[bench] plain tokens: %s\n", ids.c_str());
    }
    ...
    printf("{\"prefill_warm_ms\":%.1f,\"prefill_reps\":%d,\"label\":\"%s\",\"k\":%d,\"p_min\":%.2f,\"rewind_depth\":%d,\"prompt_tokens\":%d,"
           ...
           prefill_warm_ms, prefill_reps, getenv("LABEL") ? getenv("LABEL") : "?", K, p_min, depth, n,
