...
Env: FST_K, FST_N (node shape, m = 8), FST_LINKS (links per tenant, default 4)."""
...
def link_proc(name, host, port, reserve, seconds, q, ready, go):
    ...
    n = 0; t_end = time.time() + seconds
    while time.time() < t_end: link.pipe.exchange(frames); n += 1
    link.close(); q.put((name[0], n))
...
def phase(label, host, port, tenants, seconds):
    ...
    t0 = time.time(); go.set(); [p.join() for p in procs]; dt = time.time() - t0
    counts = {}
    while not q.empty(): t, n = q.get(); counts[t] = counts.get(t, 0) + n
