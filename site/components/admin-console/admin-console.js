...
class AdminConsole extends EnclaveElement {
  ...
  _paint() {
    ...
    /* -- TEMPORARY: stranded-fund recovery (2026-08-20) --------------------
       One-shot collection of the USDC the July/August ledger migrations left
       behind on retired ledgers. Everything is HARDCODED - record ids and
       full calldata - and every batch was verified by eth_call simulation
       from its real signer on 2026-08-20 (gas: rev-11 582,824 / rev-12
       264,279 / rev-10 163,272). DELETE this panel and its rcv- handler once
       the log shows all four green. -- */
    {
      parts.push(`<section class="ac-panel"><h3>Recover stranded funds <span class="ac-hint">temporary · 2026-08-20 · delete after use</span></h3>
        <p class="ac-sub">Refunds pay each record's OWNER and residue sweeps pay the ledger's payout wallet - the contract decides, these buttons only sign. Expected: <b>rev-11</b> sends $134.68 to governance <code>0x0b2d…eE61</code>, $11.68 to <code>0x3977…7F2A</code> and $3.93 to <code>0x081c…Ff3C</code> in one tx; <b>rev-12</b> retires + collects dust; <b>rev-10</b> refunds $1.45 to <code>0x271A…F44D</code>. Then connect <code>0x271A…F44D</code> and <code>0x3977…7F2A</code> in turn and use <b>Forward</b> to send their whole USDC balance to governance. NOT covered (known, small): rev-7's $14.63 escrow (its records still read as fundable, so <code>sweepEscrow</code> refuses - revisit if ever worth it) and ~$10.34 of operator earnings only kryptos's in-CVM key can withdraw.</p>
        <div class="ac-mig-actions">
          <button class="btn btn-sm ac-danger-btn" data-act="rcv-r11">rev-11 · retire + refund + sweep · $150.29 <span class="ac-hint">sign as 0x0b2d…eE61</span></button>
          <button class="btn btn-sm ac-danger-btn" data-act="rcv-r12">rev-12 · retire + refund dust <span class="ac-hint">sign as 0x0b2d…eE61</span></button>
          <button class="btn btn-sm" data-act="rcv-r10">rev-10 · refund $1.45 <span class="ac-hint">sign as 0x271A…F44D</span></button>
          <button class="btn btn-sm" data-act="rcv-fwd">Forward ALL USDC → governance <span class="ac-hint">sign as 0x271A…F44D, then 0x3977…7F2A</span></button>
        </div>
        <div class="ac-mig-log" id="rcvLog" role="log" aria-label="Recovery log" hidden></div></section>`);
    }
  ...
  async _onClick(e) {
    ...
    try {
      ...
      /* TEMPORARY stranded-fund recovery (2026-08-20) - hardcoded batches,
         eth_call-verified from the real signers; see the panel comment.
         DELETE together with the panel once collected. */
      if (act.startsWith("rcv-")) {
        const log = (cls, txt) => this._logTo("rcvLog", cls, txt);
        const GOV = "0x0b2d009c0c9Af05b12100D77F3c815fea822eE61";
        const RCV = {
          "rcv-r11": { to: "0x8Ce6aBCAAcc1E6EBb00dA50a46d08BbBCFc46E1f", signer: GOV, oneWay: true,
            label: "rev-11 retire + refund ×10 + sweepEscrow ×4 ($150.29 home)",
            data: "" },
          "rcv-r12": { to: "0x68f38ef8efedec5b3a0cc054fb3ffaac5639fdb7", signer: GOV, oneWay: true,
            label: "rev-12 retire + refund ×4 (dust)",
            data: "" },
          "rcv-r10": { to: "0xF7945Abc8Ae00A5689B245d203bCfCC0a459CD57", signer: "0x271A59B4B1Ee756e0E46D84aA4C9c26a8d48F44D",
            label: "rev-10 refund ×3 ($1.45 to 0x271A…F44D)",
            data: "" },
        };
        btn.disabled = true;
        try {
          await this._connect();
          ...
          if (act === "rcv-fwd") {
            if (me === GOV.toLowerCase()) { log("err", "that IS the governance wallet - connect 0x271A…F44D or 0x3977…7F2A instead"); return; }
            const bal = await balanceOf6(Enclave.address);
            if (!(bal > 0n)) { log("err", "this wallet holds no USDC - nothing to forward"); return; }
            log("p", `USDC transfer of $${(Number(bal) / 1e6).toFixed(6)} → ${GOV} - confirm in your wallet…`);
            const data = "0xa9059cbb" + GOV.slice(2).toLowerCase().padStart(64, "0") + bal.toString(16).padStart(64, "0");
            const hash = await sendTx(USDC_BASE, data);
            log("p", `  sent ${hash.slice(0, 14)}… waiting…`);
            await waitReceipt(hash, 90);
            log("ok", `forwarded $${(Number(bal) / 1e6).toFixed(2)} → governance ✓`);
            return;
          }
          const t = RCV[act];
          if (me !== t.signer.toLowerCase()) { log("err", `wrong wallet: this batch must be signed by ${t.signer} - connect it and retry`); return; }
          if (t.oneWay && !btn.dataset.armed) {
            btn.dataset.armed = "1"; btn.textContent = "Click again to PERMANENTLY retire + collect";
            log("err", "this RETIRES the ledger: one-way, no more claims/renewals/funding there, ever. It is already superseded by the live ledger, so nothing is lost. Click again to proceed.");
            return;
          }
          delete btn.dataset.armed;
          log("p", `${t.label} - confirm in your wallet…`);
          const hash = await sendTx(t.to, t.data);
          log("p", `  sent ${hash.slice(0, 14)}… waiting…`);
          await waitReceipt(hash, 120);
          log("ok", `${t.label} ✓`);
        } catch (err) { log("err", friendly(err) + " - safe to retry: each batch is one atomic tx (all-or-nothing), and a re-send of a completed batch reverts harmlessly"); }
        finally { btn.disabled = false; }
        return;
      }
