...
//   2. the instance SECRET never leaves the payload: every use of g_isk in anchor_payload.c is one of the five allowed ones
//      (its declaration, its derivation, the v3 evidence signature, this signature, arming the re-attach context), and INSTANCEATTACH prints only the
...
test("the instance SECRET never leaves the payload: g_isk used only in its five allowed places; INSTANCEATTACH prints only the SPKI and the signature", () => {
  ...
  const allowed = [/static unsigned char g_ipk\[32\], g_isk\[64\];/, /crypto_sign_ed25519_tweet_seed_keypair\(g_ipk, g_isk, seed\)/,
                   ...
                   /anchor_reattach_arm\(&g_reattach, g_tpk, g_ppk, g_isk, g_inst\);/];   // the re-attach context keeps a POINTER (anchor_reattach.h)
  ...
  assert.equal(uses.length, 5, `exactly the five allowed uses (${uses.map(([n]) => n).join(",")})`);
