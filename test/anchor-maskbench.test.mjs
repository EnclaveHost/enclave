...
test('MASKBENCH comparator copies a host-minted public shipment under a temp store, verifies, times and removes it; bad inputs fail without timing lines', () => {
  ...
  try {
    ...
    const objs = [];
    for (const [src, strict] of [[join(root, 'test/fixtures/anchor-maskbench.c'), true], [join(payload, 'anchor_maskbench.c'), true], [join(payload, 'anchor_maskbench_mint.c'), true],
        [join(gg, 'shielded-pads.c'), false], [join(gg, 'shielded-field.c'), false], [join(payload, 'third_party/tweetnacl.c'), false], [join(gg, 'poly1305-donna.c'), false]]) {
