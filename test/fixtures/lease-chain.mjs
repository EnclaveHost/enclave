...
export async function startLeaseChain({ port = 18545 + Math.floor(Math.random() * 400), chainId = 31337 } = {}) {
  const V = await import("viem");
  const C = await compileContracts();
  const anvil = spawn(ANVIL, ["--port", String(port), "--chain-id", String(chainId), "--disable-code-size-limit", "--silent"], { stdio: ["ignore", "pipe", "pipe"] });
  ...
  const [deployer, operator, tenant, payout, stranger] = await pub.request({ method: "eth_accounts" });   // anvil's unlocked accounts
  ...
  return {
    rpc, chainId, accounts: { deployer, operator, tenant, stranger }, addresses: { registry, ledger, proofOfTime: pot, usdc },
