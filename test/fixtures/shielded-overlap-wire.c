...
typedef struct { int fd, ready, release, count, mode; } peer;
typedef struct { int ready, release, calls; pthread_t caller; } work;
...
static void *serve(void *arg) {
    ...
    for (int i = 0; i < p->count; i++) {
        ...
        assert(h[0] == 42 && get_u64(h + 1) == sizeof payload);
...
static void run_case(int count, int mode) {
    ...
    peer server = { sockets[1], ready[1], release[0], count, mode };
    work w = { ready[0], release[1], 0, pthread_self() };
    pthread_t thread; assert(pthread_create(&thread, NULL, serve, &server) == 0);
    sh_frame frames[20]; sh_reply replies[20];
    for (int i = 0; i < count; i++) frames[i] = (sh_frame){42, "hel", 3, "lo", 2};
    int rc = sh_pipe_exchange_work(p, frames, count, replies, do_work, &w);
    assert(w.calls == 1);
    assert(rc == (mode == 0 ? SH_OK : mode == 1 ? SH_ERR_VIOLATION : mode == 2 ? SH_ERR_PROTO : SH_ERR_IO));
    for (int i = 0; i < count; i++) {
        if (!mode) assert(replies[i].len == 5 && !memcmp(replies[i].data, "reply", 5));
