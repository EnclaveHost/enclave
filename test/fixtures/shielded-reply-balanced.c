...
typedef struct { const char *name; bool (*fn)(const int32_t *, size_t); } table;
...
int main(void) {
    const table tables[] = {
        { "generic", sh_simd_generic_reply32_balanced },
#if !defined(__aarch64__)
        { "avx512",  sh_simd_avx512_reply32_balanced  },
#else
        { "neon",    sh_simd_neon_reply32_balanced    },
#endif
    };
    ...
    for (size_t t = 0; t < sizeof tables / sizeof tables[0]; t++) {
        ...
        free(b);
        fprintf(stderr, "reply-balanced: %s ok\n", T->name);
    }
    return 0;
}
