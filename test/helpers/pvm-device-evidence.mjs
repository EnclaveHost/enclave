...
export function readExchanges(dir) {
  ...
  return names.map((f) => {
    ...
    return { n, nonce: m ? m[1] : null, requestText, envelope, envelopeText, parseError };
  });
}

/** The client's expectations for one exchange, from the committed policy body and the request's nonce. */
export function expectFromPolicy(policyBody, nonceHex) {
 ...
 */
export async function reverifyExchange(ex, policyBody, { now }) {
  if (!ex.nonce) return { ok: false, why: `request is not "EVIDENCE <nonce>": ${JSON.stringify(ex.requestText.slice(0, 40))}` };
  if (!ex.envelope) return { ok: false, why: `envelope unreadable: ${ex.parseError}` };
  const expect = expectFromPolicy(policyBody, ex.nonce);
