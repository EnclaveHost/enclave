...
for (const fortify of [0, 3]) test(`signed prefix snapshot survives source mutation and refuses corrupt or oversized reads (fortify ${fortify})`, () => {
  const dir = mkdtempSync(join(tmpdir(), 'prefix-snapshot-'));
  const gg = fileURLToPath(new URL('../wasm/ggml-shielded/', import.meta.url));
  const flags = ['-D_GNU_SOURCE', '-U_FORTIFY_SOURCE', `-D_FORTIFY_SOURCE=${fortify}`, '-DSH_PREFIX_TEST_IO', '-std=c11', '-O1', '-g',
  ...
  try {
    ...
    run('cc', [...flags, ...objs, '-Wl,--gc-sections', '-Wl,--wrap=pread', '-Wl,--wrap=__pread_chk', '-o', bin]);
