// OFAC routing through the live indexer path (split from
// relay-indexer.test.mjs so each file fits the suite's per-file timeout):
// an SDN-listed payer and absent screening data must both land in review,
// never auto-provision; a reviewer's reject is terminal.
...
test("indexer: an SDN-listed payer never auto-provisions; reviewer approval releases it", async (t) => {
  ...
  state.logs.push(paymentLog(order.ref, SANCTIONED, 1803600n, 101n));
  state.tip = 103n;
  ...
  const item = rq.body.items.find((i) => i.orderId === order.id && i.reason === "ofac_hit");
  assert.ok(item, "the OFAC hit opened a review item");

  // a reviewer REJECTing keeps it terminal-refused
  const rr = await api(origin, "POST", `/v1/billing/review/${item.id}/resolve`,
    { headers: { "x-admin-token": "admintok" }, body: { action: "reject", note: "sdn match" } });
  assert.equal(rr.status, 200);
  const after = await api(origin, "GET", `/v1/billing/orders/${order.id}`, { token });
  assert.equal(after.body.state, "rejected");
});

test("indexer: screening data absent means stale, which reviews rather than provisions", async (t) => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "relay-idx-"));
  // NO seedOfac: cache absent and the fetch URL is dead -> "stale"
  const state = { tip: 100n, logs: [] };
  const { origin } = await startStack(t, { dataDir: dir, state });
  const token = await login(origin);
  const order = await makeOrder(origin, token);
  ...
  await waitState(origin, token, order.id, "under_review");
  const rq = await api(origin, "GET", "/v1/billing/review", { headers: { "x-admin-token": "admintok" } });
  assert.equal(rq.body.items.filter((i) => i.orderId === order.id && i.reason === "screening_stale").length, 1);
});
