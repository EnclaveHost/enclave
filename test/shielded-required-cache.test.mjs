...
test('required worker cache releases authenticated rows after private checks and refuses missing prerequisites',t=>{
 ...
 const dir=mkdtempSync(join(tmpdir(),'shielded-required-cache-'));
 try {
  const objects=[];
  ...
  run('c++',[...flags,'-std=c++17','-I'+join(headers,'ggml/include'),'-I'+join(headers,'ggml/src'),
   join(root,'test/fixtures/shielded-required-cache.cpp'),...objects,'-Wl,--gc-sections',
   '-L'+libs,'-lggml','-lggml-cpu','-lggml-base','-lpthread','-lm','-Wl,-rpath,'+libs,'-o',bin]);
  for(const mode of ['off','on','unauth','nopad','nocache','nodealt','malformed']) assert.match(execFileSync(bin,[dir,mode],{encoding:'utf8',timeout:60000,env}),/required-cache:.*PASS/);
