...
test('source prefetch overlaps the next read with authenticated registration and preserves failure and lifetime rules',t=>{
 ...
 const dir=mkdtempSync(join(tmpdir(),'shielded-source-prefetch-'));
 try {
  const objects=[];
  ...
  run('c++',[...flags,'-std=c++17','-I'+join(headers,'ggml/include'),'-I'+join(headers,'ggml/src'),
   join(root,'test/fixtures/shielded-source-prefetch.cpp'),...objects,'-Wl,--gc-sections',
   '-L'+libs,'-lggml','-lggml-cpu','-lggml-base','-lpthread','-lm','-Wl,-rpath,'+libs,'-o',bin]);
  for(const mode of ['off','on','tamper','readfail','abort','lifetime','mismatch','allocfail','cap','invalid','encoded']) assert.match(execFileSync(bin,[dir,mode],{encoding:'utf8',timeout:60000,env}),/source-prefetch:.*PASS/);
