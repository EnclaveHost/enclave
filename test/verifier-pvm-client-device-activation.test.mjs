// Independent review of the pVM owner's bounded Pixel 10 run of the installed client's activation (their a7d624c2,
// results/pvm-cpu-client-activation, copied verbatim into test/fixtures/verifier/pvm-client-activation-device with
// SOURCES.json). Nothing here reuses the owner's checker: the artifact identity is checked against this branch's own
// reproduction (verifier/integration/next-builds.json) and pins, the manifests' signatures and the policies are verified
// with this session's code, the generation log and every snapshot are walked for monotonicity and key preservation, the
// delegated runs for commit-before-request and running identity, the refusals for their named records, and the VM capture
// is decoded for the served count. The run's own checker output FAILED on a miscount and is kept; the count is redone here.
// EVIDENCE CLASSES: what rests on the device is the activated 0.3.1's own result lines (its verified summary: format,
// app, runtime, code hash, keys) plus the VM capture's served count; the raw evidence envelopes are NOT in the results,
// so this session did not re-verify the attestation chains offline for this run, and stream authenticity rests on the
// client's FIN verification plus the served count (the installed CLI writes no session secrets). Stated as such.
...
const F = new URL("./fixtures/verifier/pvm-client-activation-device/", import.meta.url).pathname;
...
const keys = { policy: js("policy-key.json"), release: js("release-key.json"), successor: js("successor-key.json") };
const anchor = js("cli-install.json").anchor;
...
test("the fixture is the owner's results verbatim: every file hashes to SOURCES.json, and the five hashes the owner reported match", () => {
  ...
  assert.equal(Object.keys(SRC.files).length, 137);
});
test("artifact identity: the activated 0.3.1 is this branch's reproduced derivation of the pinned 0.3.0 dist, and both manifests name exactly it", () => {
...
test("the generation log: 7 generations, monotonic serial, digests equal to the accepted policies' bytes, release key constant, staged then active equal to the signed record, rotation recorded, nothing ever moved back", () => {
...
test("snapshots: activation added exactly one generation; from the rotation on, every snapshot (tamper, refused repairs, repairs, missing, rollback, final) is generation 7 with the same state", () => {
...
test("the six policies replayed with this session's verifier, under the anchor and the rollback memory the state names, give the device's outcomes", () => {
...
test("running identity and order: before activation 0.3.0 answers; after it every answered run is 0.3.1 (ten results, the checker's hand count of nine was the miscount), each delegated run committed its policy before its evidence request, and each verified summary names the policy's pins", () => {
  ...
  assert.match(c0, /^FAIL after activation every answered run was 0\.3\.1, never 0\.3\.0 \(10 results\)$/m, "the run's own checker failed exactly on the count"); assert.match(c0, /^FAIL \(1\)$/m);
  assert.match(c1, /^ok   after activation every answered run was 0\.3\.1, never 0\.3\.0 \(10 results, want 10\)$/m); assert.match(c1, /^PASS /m); assert.equal(/^FAIL/m.test(c1), false);
...
test("refusals and repairs: tamper in place (same inode), refused at launch with expected and the tampered digest as found, nothing else printed; missing: found missing; the wrong file cannot be overwritten; the retired key cannot repair; the successor's manifest re-publishes (new inode, read-only), twice", () => {
...
test("the VM capture, decoded here: the VM served exactly the released requests (9 streams to FIN, 1 whole), and no request or token appears in the clear in the capture, the hub or the carriers", () => {
...
test("evidence classes, as this review states them: device evidence is the activated client's own verified summaries plus the VM's served count; the raw evidence envelopes are not in the results, so no attestation chain was re-verified offline here", () => {
  for (const l of ["active-stream", "active-whole", "planted-marker", "active-policy-2", "rotate-3", "successor-4", "repaired-stream", "repaired-2-stream"]) assert.equal(typeof result(l).verified?.key, "string", `${l}: a verified summary only`);
  assert.equal(fs.existsSync(path.join(F, "evidence")), false, "no envelopes saved"); assert.equal(Object.keys(SRC.files).some((f) => /evidence|envelope|trace/i.test(f)), false);
  assert.match(rd("NOTES.md").toString(), /no offline-reopenable stream traces/);
});
