...
const K = keys();
let L; test.before(async () => { L = await labServer(); }); test.after(() => { L?.close(); fs.rmSync(tmp, { recursive: true, force: true }); });
...
test("one key alone cannot ship code: a wrong countersignature, or a stranger's release key, is refused and nothing is staged", { skip }, async () => {
  ...
  const bytes = fakeArtifact("0.3.0"); L.artifacts.set("u2", { bytes });
  const good = signedManifest(K, "0.3.0", bytes);
  L.manifests.set("u2", { ...good, policySig: signedManifest(K, "0.3.0", bytes, { policyKey: K.other }).policySig });
...
test("release-key rotation and a policy commit, both orders: neither commit loses the other's fields; the successor release key then signs, a stranger does not", { skip }, async () => {
  ...
  put("u5", "0.3.0", { hold: true, nextReleaseKey: rawPub(K.release2).toString("hex") }); L.policies.set("p7", signedPolicy(K, 7)); L.policies.set("p8", signedPolicy(K, 8));
  ...
  assert.equal(st?.serial, 7, "the policy's serial survived the update's commit"); assert.equal(st?.staged?.version, "0.3.0"); assert.equal(st?.nextReleaseFp, fpOf(K.release2), "the release-key successor is recorded");
  await runToEvidence(d, "p8", "u5-q");                                                     // and a policy commit after the update
  st = committedState(CLI, tmp, d.state); assert.equal(st?.serial, 8); assert.equal(st?.staged?.version, "0.3.0", "the staging survived the policy's commit"); assert.equal(st?.nextReleaseFp, fpOf(K.release2));
  put("u5b", "0.4.0", { releaseKey: K.release2 }); const r2 = await update(d, "u5b").done; assert.equal(r2.status, 0, r2.out); assert.equal(staged(d)?.version, "0.4.0");
  ...
  put("u5c", "0.5.0", { releaseKey: K.release }); const r3 = await update(d, "u5c").done; assert.notEqual(r3.status, 0, "the retired release key no longer signs");
  put("u5d", "0.5.0", { releaseKey: K.other }); const r4 = await update(d, "u5d").done; assert.notEqual(r4.status, 0);
  assert.equal(staged(d)?.version, "0.4.0");
});
test("a storm of concurrent updates and policy commits: whatever the interleaving, the final state is the highest version and serial, every loser was refused by the monotonic rule, and the generation count equals the commits", { skip }, async () => {
  ...
  const versions = [0, 1, 2, 3, 4, 5].map((i) => `0.3.${i}`), serials = [10, 11, 12, 13, 14, 15];
  ...
  assert.equal(st?.serial, 15); assert.equal(staged(d)?.version, "0.3.5"); assert.equal(staged(d)?.bytesMatch, true);
  let commits = 0;
  for (const [i, u] of ur.entries()) { if (u.status === 0) commits++; else assert.match(u.update?.reasons?.[0] || "", /is already staged: 0\.3\.\d cannot replace it/, `update ${versions[i]}: ${u.out}`); }
