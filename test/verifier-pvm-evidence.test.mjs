...
import { verifyEvidence, verifyPvmEvidence, loadOwnerVerifier, admit, createNonceRegistry, RELEASE, HOLD, PVM_EVIDENCE_FORMAT } from "../verifier/index.mjs";
...
const standIn = async (env, o) => {
  ...
  const allowed = new Set(["format", "nonce", "app", "spki", "identity", "selftest", "chain", "tlsKey", "appKey"]);
  ...
  return { ok: true, reasons, transportSpki: env.spki, runtimeId: rid, appKey: env.appKey ?? null };
};
...
test("browser client: holds until the evidence carries an application-layer key; TLS pinning is never claimed", async () => {
  ...
  const v2 = await run(evidenceFor(nonce2, { appKey: "ab".repeat(32) }), expectFor(nonce2));
...
test("malformed evidence: wrong format, extra fields, not an object, oversized", async () => {
  const nonce = randomBytes(32);
  await rejected({ ...evidenceFor(nonce), format: "enclave-pvm-app-evidence/v0" }, expectFor(nonce), /is not enclave-pvm-app-evidence\/v1/);
