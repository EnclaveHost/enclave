...
import { Manager, createServer } from "../windows/vbslike/manager/server.mjs";
import { HyperVPartitionBackend } from "../windows/vbslike/manager/backend.mjs";
import { OWNER_MARKER } from "../windows/vbslike/manager/wmi-launcher.mjs";
...
class FakeLauncher {
  ...
  async start(mapping, { instanceId, notes } = {}) {
  ...
  async stop(handle) {
    ...
    const vm = this.host.vms.get(handle.name);
    if (vm) this.host.vms.delete(handle.name);                 // Stop-VM -TurnOff, then gone from our view
  ...
  async survey() {
    ...
    return { vms: [...this.host.vms.values()].filter((x) => x.name.startsWith(this.prefix))
                   .map((x) => ({ name: x.name, state: x.state, notes: x.notes })) };
  }
...
/** Boot a manager the way main.mjs does: construct, probe(), listen on loopback. */
...
// ---- BUG REPRODUCTION (today's behaviour; expected to FAIL once the fix lands - delete then) ----

test("BUG REPRODUCTION P1: retire(instanceId) after a manager restart releases the lease while the VM runs", async () => {
  const host = new FakeHost();
  const { m1, client, instanceId } = await running(host);
  await restartManager(m1, host);
  const led = ledger();
  const r = await retire({ client, deployment, ledger: led, instanceId });
  assert.equal(r.removed, true);
  assert.equal(r.leaseFree, true);
  assert.deepEqual(led.released.map((x) => x.why), ["the domain was removed and confirmed gone"]);
  assert.equal(host.running().length, 1, "...and the VM is still running on the host");
});

test("BUG REPRODUCTION P1b: retire(no id) after a manager restart releases the lease while the VM runs", async () => {
  const host = new FakeHost();
  const { m1, client } = await running(host);
  await restartManager(m1, host);
  const led = ledger();
  const r = await retire({ client, deployment, ledger: led });
  assert.equal(r.leaseFree, true);
  assert.deepEqual(led.released.map((x) => x.why), ["no domain for this deployment"]);
  assert.equal(host.running().length, 1, "...and the VM is still running on the host");
});

test("BUG REPRODUCTION: reconcile after a manager restart starts a SECOND VM for the same deployment", async () => {
  const host = new FakeHost();
  const { m1, client } = await running(host);
  await restartManager(m1, host);
  const r = await reconcile({ client, deployment, ...fast });
  assert.equal(r.action, "spawned", "the surviving VM was not adopted");
  assert.equal(host.running().length, 2, "two VMs now run for one deployment");
});

test("BUG REPRODUCTION P1c: a manager restart while reconcile waits releases the lease while the VM runs", async () => {
  const host = new FakeHost();
  let m = await bootManager(host, 0, { judgeReady: () => new Promise(() => {}) });   // stays `starting`
  const client = clientFor(m.port);
  const led = ledger();
  let restarted = false;
  const r = await reconcile({ client, deployment, ledger: led, pollMs: 5, deadlineMs: 5_000,
    sleep: async () => { if (!restarted) { restarted = true; m = await restartManager(m, host); } } });
  assert.equal(r.action, "failed");
  assert.equal(r.leaseFree, true);
  assert.match(r.reason, /disappeared from the manager/);
  assert.equal(host.running().length, 1, "...and the VM is still running on the host");
});

// ---- INVARIANT (todo until the manager fix, d1, and/or the node lifecycle fix, 5d, land) ----------

const TODO = "the manager restart fix (d1: epoch + stale 409/410 + inventory rebuilt from Hyper-V) and the node side (5d: unknown-to-a-younger-manager is HELD)";
...
test("INVARIANT: reconcile after a restart adopts or holds, and never starts a second VM", { todo: TODO }, async () => {
  const host = new FakeHost();
  const { m1, client } = await running(host);
