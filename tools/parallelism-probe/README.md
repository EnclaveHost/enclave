...
| `robust-cycle.c` | the death hook and `__pthread_exit` BOTH walk `robust_list` following guest-writable `_m_next` pointers while `__thread_list_lock` is held; a forged self-cycle wedges every sibling's `pthread_create`/`pthread_join`. Two arms selected by a GUEST ARGV word after the wasm file (`… robust-cycle.wasm trap` / `… exit`), NOT an env var — round 14 bounded only the trap twin, so r14a hangs the `exit` arm. r14b passes both. Note musl links every non-`NORMAL` mutex into `robust_list`, so a plain `PTHREAD_MUTEX_RECURSIVE` populates it: not exotic |
| `site2-refusal-reach.c` | the adversarial probe behind round 17's `fgetwc` verdict, committed in round 18 after round 18 pointed out it never was. An owner refilling the shared buffer interleaved with a non-owner reading it — 1,189 `fgetwc` calls and **6** refusals (six sample events — the header's earlier "hundreds … at every offset inside a 3-byte character" was wrong, and the probe emits no offset data to support it), on **entirely valid UTF-8** so any owner `F_ERR` could only have been manufactured by the split. r14a FAIL, r14c/r14d PASS, native PASS. **Read its header before citing it**: round 18's row claimed this "proves a refusal cannot reach the site-2 EILSEQ branch" and round 19 showed it does not — its r14a FAIL is a SITE-1 signal, and at site 2 it is byte-identical on r14b and r14c, the only two images that differ there. It is an unfalsified null at site 2, not a proof; `wide-read-eilseq.c` is the probe that discriminates there |
