;; SET benchmark: run(n, iters) spawns n REAL threads via thread.spawn-ref,
;; each burns `iters` iterations of an LCG against a per-thread slot in the
;; SHARED memory, then atomically bumps a completion counter and notifies.
;; The main thread futex-waits until all n have finished. Falls back to
;; inline execution if spawn reports failure, so the result is correct on
;; any engine; only the timing differs.
;;
;; Shared memory layout:
;;   0:  i32 completion counter (atomic)
;;   16: i64 iteration count (published by main before spawning)
;;   64: i64 per-thread result slots (proves each ctx arrived + defeats DCE)
;;
;;   wasmtime run -W threads,shared-everything-threads,component-model-threading,shared-memory \
;;     --invoke 'run(16, 900000000)' set-spawn-parallel.wat
(component
  (core type $start (shared (func (param i32))))
  (core func $spawn (canon thread.spawn-ref $start))
  (core func $ap    (canon thread.available_parallelism))

  (core module $m
    (type $start_t (shared (func (param i32))))
    (type $ap_t    (shared (func (result i32))))
    (type $spawn_t (shared (func (param (ref null $start_t) i32) (result i32))))

    (import "" "spawn" (func $spawn (type $spawn_t)))
    (import "" "ap"    (func $ap    (type $ap_t)))

    (memory (export "memory") 1 1 shared)

    (func $worker (type $start_t) (param $ctx i32)
      (local $i i64)
      (local $acc i64)
      (local.set $i (i64.load (i32.const 16)))
      (block $done
        (loop $l
          (br_if $done (i64.eqz (local.get $i)))
          (local.set $acc
            (i64.add
              (i64.mul (local.get $acc) (i64.const 6364136223846793005))
              (i64.const 1442695040888963407)))
          (local.set $i (i64.sub (local.get $i) (i64.const 1)))
          (br $l)))
      ;; sink the accumulator so the loop cannot be optimized away
      (i64.store
        (i32.add (i32.const 64) (i32.mul (local.get $ctx) (i32.const 8)))
        (local.get $acc))
      (drop (i32.atomic.rmw.add (i32.const 0) (i32.const 1)))
      (drop (memory.atomic.notify (i32.const 0) (i32.const -1))))
    (elem declare func $worker)

    (func (export "run") (param $threads i32) (param $iters i64) (result i32)
      (local $k i32)
      (local $rc i32)
      (local $c i32)
      (i64.store (i32.const 16) (local.get $iters))
      (block $spawned
        (loop $s
          (br_if $spawned (i32.ge_u (local.get $k) (local.get $threads)))
          (local.set $rc (call $spawn (ref.func $worker) (local.get $k)))
          (if (i32.lt_s (local.get $rc) (i32.const 0))
            (then (call $worker (local.get $k))))
          (local.set $k (i32.add (local.get $k) (i32.const 1)))
          (br $s)))
      ;; join: block until every worker has bumped the counter
      (block $done
        (loop $w
          (local.set $c (i32.atomic.load (i32.const 0)))
          (br_if $done (i32.eq (local.get $c) (local.get $threads)))
          (drop (memory.atomic.wait32 (i32.const 0) (local.get $c) (i64.const -1)))
          (br $w)))
      (i32.atomic.load (i32.const 0)))
  )
  (core instance $i (instantiate $m (with "" (instance
    (export "spawn" (func $spawn))
    (export "ap" (func $ap))))))
  (alias core export $i "run" (core func $run))
  (func (export "run") (param "n" u32) (param "iters" u64) (result u32)
    (canon lift (core func $run)))
)
