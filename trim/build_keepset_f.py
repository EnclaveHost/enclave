"""build_keepset_f.py <MIN_FREQ> <out_dir> [ascii]: frequency-only keep set (no printable-ASCII rule unless "ascii"). Build the keep set from the frequency table + rules, close it under BPE merge parents,
...
try:
    ...
    class ud:  # histogram only; script() unavailable
        @staticmethod
        def script(ch): return "Latin" if ch.isascii() else "NonASCII"
HERE = os.path.dirname(os.path.abspath(__file__))
SNAP = "/home/steven/Documents/Codex/2026-09-07/i-w/work/tensor-sdk/hf/gemma-4-E2B-it/"
import sys
ART = os.path.join(HERE, "artifacts")            # frequency table source
MIN_FREQ = int(sys.argv[1]); OUT = sys.argv[2]; KEEP_ASCII = len(sys.argv) > 3 and sys.argv[3] == "ascii"
...
for i, p in inv.items():
    if KEEP_ASCII and p and all(c in PRINTABLE for c in p): reasons[i].add("ascii")
...
print(f"vocab {V}: freq>={MIN_FREQ} {int((freq>=MIN_FREQ).sum())}, +ascii/byte/special -> {len(before_closure)}, +merge-parent closure -> {len(keep)}")

keep_ids = np.array(sorted(keep), dtype=np.int64)
np.save(os.path.join(OUT, "keep_ids.npy"), keep_ids)
id_map = {int(o): n for n, o in enumerate(keep_ids)}
json.dump(id_map, open(os.path.join(OUT, "id_map_old_to_new.json"), "w"))
with open(os.path.join(OUT, "keep_reasons.tsv"), "w") as fh:
...
json.dump(summary, open(os.path.join(OUT, "keepset_summary.json"), "w"), indent=1, ensure_ascii=False)
