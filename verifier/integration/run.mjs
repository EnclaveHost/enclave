...
// the Node consumer bundle (verifier/dist, vendored for the relay) must rebuild byte for byte from this tree too
const na = spawnSync(process.execPath, [path.join(REPO, "verifier", "node", "reproduce.mjs")], { cwd: REPO, encoding: "utf8" });
if (na.status !== 0) { process.stderr.write(na.stderr || ""); console.error("integration: the Node consumer bundle did NOT reproduce; refusing"); process.exit(2); }
process.stdout.write(na.stdout.split("\n").filter((l) => /REPRODUCED/.test(l)).map((l) => `integration: ${l}\n`).join(""));
