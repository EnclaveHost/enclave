...
/* Single-card paired experiment, selected outside decode. The same locks as
 * graph execution prevent socket-option changes during an exchange. Refuse a
 * pool rather than partially applying the experiment to several cards. */
extern "C" int ggml_backend_shielded_set_rcvlowat(int cap, uint64_t *buffer_bytes) {
    sh_pool &p = sh_pool_get();
    std::lock_guard<std::mutex> lock(p.mu);
