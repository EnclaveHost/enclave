...
static int body_lines(char *out, size_t cap, const uint8_t model_digest[32], const uint8_t *calib_digest, const uint8_t prefix_h[64], uint64_t n_tokens, const uint8_t file_h[64]) {
    char md[65], ph[129], fh[129];
    sh_pads_bin2hex(model_digest, 32, md); sh_pads_bin2hex(prefix_h, 64, ph); sh_pads_bin2hex(file_h, 64, fh);
    char cd[65]; if (calib_digest) sh_pads_bin2hex(calib_digest, 32, cd);
    const int n = calib_digest ?
        snprintf(out, cap, "enclave-prefix-kv-v2\nmodel-sha256 %s\ncalib-sha512-256 %s\nprefix-sha512 %s\ntokens %llu\nfile-sha512 %s\n", md, cd, ph, (unsigned long long)n_tokens, fh) :
        snprintf(out, cap, "enclave-prefix-kv-v1\nmodel %s\nprefix-sha512 %s\ntokens %llu\nfile-sha512 %s\n", md, ph, (unsigned long long)n_tokens, fh);
    return n > 0 && (size_t)n < cap ? n : -1;
}

static int prefix_sign(const char *kv_path, const uint8_t model_digest[32], const uint8_t *calib_digest, const char *prefix, size_t prefix_len,
    ...
    char body[512];
    const int n = body_lines(body, sizeof body, model_digest, calib_digest, ph, n_tokens, fh);
    ...
    int bad = fputs(body, f) < 0 || fprintf(f, "sig %s\n", sig_hex) < 0;
    if (fflush(f) != 0) bad = 1;
    if (fclose(f) != 0) bad = 1;
    if (bad) { snprintf(err, err_cap, "cannot write complete prefix sidecar"); return -1; }
    return 0;
}
int sh_prefix_kv_sign(const char *kv_path, const uint8_t model_digest[32], const char *prefix, size_t prefix_len,
                      uint64_t n_tokens, const uint8_t sk[64], char *err, size_t err_cap) {
    return prefix_sign(kv_path, model_digest, NULL, prefix, prefix_len, n_tokens, sk, err, err_cap);
}
int sh_prefix_kv_sign_v2(const char *kv_path, const uint8_t model_sha256[32], const uint8_t calib_digest[32], const char *prefix, size_t prefix_len,
                         uint64_t n_tokens, const uint8_t sk[64], char *err, size_t err_cap) {
    if (!model_sha256 || !calib_digest) { snprintf(err, err_cap, "missing model or calibration identity"); return -1; }
    return prefix_sign(kv_path, model_sha256, calib_digest, prefix, prefix_len, n_tokens, sk, err, err_cap);
}
...
static int prefix_snapshot_read(const char *kv_path, int kv_fd, const uint8_t pk[32], const uint8_t model_digest[32], const uint8_t *calib_digest, const char *prefix, size_t prefix_len,
    ...
    char side[1024]; size_t got = fread(side, 1, sizeof side - 1, f); int bad = ferror(f); fclose(f);
    if (bad || got == sizeof side - 1 || memchr(side, 0, got)) { snprintf(err, err_cap, "unreadable or noncanonical sidecar"); return -1; }
    side[got] = 0;
    ...
    if (got != body_len + 4 + 128 + 1 || side[got - 1] != '\n') { snprintf(err, err_cap, "noncanonical sidecar ending"); return -1; }
    ...
    char md[65] = "", cd[65] = "", ph[129] = "", fh[129] = ""; unsigned long long ntok = 0;
    int parsed = 0;
    if (calib_digest) {
        parsed = !strncmp(side, "enclave-prefix-kv-v2\n", 21) &&
            sscanf(side, "enclave-prefix-kv-v2\nmodel-sha256 %64s\ncalib-sha512-256 %64s\nprefix-sha512 %128s\ntokens %llu\nfile-sha512 %128s\n", md, cd, ph, &ntok, fh) == 5;
    } else {
        parsed = !strncmp(side, "enclave-prefix-kv-v1\n", 21) &&
            sscanf(side, "enclave-prefix-kv-v1\nmodel %64s\nprefix-sha512 %128s\ntokens %llu\nfile-sha512 %128s\n", md, ph, &ntok, fh) == 4;
    }
    if (!parsed) { snprintf(err, err_cap, "malformed sidecar or wrong version (requires v%d)", calib_digest ? 2 : 1); return -1; }
    ...
    if (calib_digest) {
        sh_pads_bin2hex(calib_digest, 32, hex);
        if (strcmp(hex, cd)) { snprintf(err, err_cap, "signed for another calibration"); return -1; }
    }
    ...
    uint8_t signed_file_hash[64]; char canonical[512];
    if (strlen(fh) != 128 || !sh_pads_hex2bin(fh, signed_file_hash, 64)) { snprintf(err, err_cap, "invalid signed file hash"); return -1; }
    const int canonical_size = body_lines(canonical, sizeof canonical, model_digest, calib_digest, want_ph, ntok, signed_file_hash);
    if (canonical_size < 0 || (size_t)canonical_size != body_len || memcmp(side, canonical, body_len)) {
        snprintf(err, err_cap, "noncanonical signed sidecar body"); return -1;
    }
...
int sh_prefix_kv_snapshot_read(const char *kv_path, int kv_fd, const uint8_t pk[32], const uint8_t model_digest[32], const char *prefix, size_t prefix_len,
                               size_t max_bytes, uint64_t max_tokens, sh_prefix_kv_snapshot *out, char *err, size_t err_cap) {
    return prefix_snapshot_read(kv_path, kv_fd, pk, model_digest, NULL, prefix, prefix_len, max_bytes, max_tokens, out, err, err_cap);
}
int sh_prefix_kv_snapshot_read_v2(const char *kv_path, int kv_fd, const uint8_t pk[32], const uint8_t model_sha256[32], const uint8_t calib_digest[32], const char *prefix, size_t prefix_len,
                                  size_t max_bytes, uint64_t max_tokens, sh_prefix_kv_snapshot *out, char *err, size_t err_cap) {
    if (!model_sha256 || !calib_digest) {
        if (out) memset(out, 0, sizeof *out);
        snprintf(err, err_cap, "missing model or calibration identity"); return -1;
    }
    return prefix_snapshot_read(kv_path, kv_fd, pk, model_sha256, calib_digest, prefix, prefix_len, max_bytes, max_tokens, out, err, err_cap);
}
