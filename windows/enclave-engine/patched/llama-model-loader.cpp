...
// PATCHED for the VBS enclave build (windows/enclave-engine): the optional tensor validation ran on std::async,
// whose MSVC implementation is the PPL thread pool and exception_ptr, neither of which exists in VTL1. The
// validation (check_tensors, off by default) runs synchronously instead; nothing else changes.
namespace ee { template <class T> struct future { T v; T get() { return v; } }; }
template <class F> auto ee_async(F && f) -> ee::future<decltype(f())> { return { f() }; }
#else
#include <future>
template <class F> auto ee_async(F && f) { return std::async(std::launch::async, std::forward<F>(f)); }
#endif
#include <regex>
...
bool llama_model_loader::load_all_data(
    ...
    #ifdef __ENCLAVE_PROJECT__
    std::vector<ee::future<std::pair<ggml_tensor *, bool>>> validation_result;
#else
    ...
    for (struct ggml_tensor * cur = ggml_get_first_tensor(ctx); cur != NULL; cur = ggml_get_next_tensor(ctx, cur)) {
        ...
        if (use_mmap) {
            ...
            if (check_tensors) {
                validation_result.emplace_back(ee_async([cur, data, n_size] {
            ...
            if (ggml_backend_buffer_is_host(cur->buffer)) {
                ...
                if (check_tensors) {
                    validation_result.emplace_back(ee_async([cur, n_size] {
