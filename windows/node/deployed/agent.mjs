...
async function handle(frame) {
  ...
  if (APPS && method === 'HEAD' && /^\/x\/0x[0-9a-fA-F]{64}\/?$/.test(p)) {
    ...
    // holdsLease, NOT records.has: records carry every deployment ever considered, refusals
    // included, and answering 204 for one told the relay another box's tenant belonged here.
    return host.holdsLease(id) ? { status: 204, headers: {}, body: '' } : json(404, { error: 'not_found', id });
  }
  if (APPS && /^\/x\/0x[0-9a-fA-F]{64}(\/|$)/.test(p)) {
    const id = p.split('/')[2].toLowerCase();
    // The same rule on the serving path: a misrouted request gets 404, not a 503 about this box's
    // own policy, which reads as "your app is broken here" when it was never here.
