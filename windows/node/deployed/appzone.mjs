  ...
  async function onHead(st, head) {
    ...
    wss.handleUpgrade(req, st.sock, head.rest, (ws) => {
      ...
      tlsSock.on("secure", () => {
        ...
        // AN ANSWER, NOT A CLOSED SOCKET. abort() destroys the TLS socket, and with nothing yet
        // written a browser renders that as ERR_EMPTY_RESPONSE - a blank tab. It happened for real:
        // two apps stopped accepting on their brokered port and their own hostnames went blank.
        // A failure BEFORE the app speaks now answers 502 with a body. FLUSH THEN CLOSE: the write
        // is ENDED and nothing else touched, so the TLS socket's own close runs finish() exactly as
        // a normal response does; the timer is only a backstop for a peer that never closes.
        let spoke = false;
        const refuse = (why) => {
          log(`app-zone ${id.slice(0, 10)}: ${why}`);
          try { if (app) app.destroy(); } catch {}
          ...
          const guard = setTimeout(() => {
            log(`app-zone ${id.slice(0, 10)}: the 502 did not flush within 10000ms; closing`);
            try { tlsSock.destroy(); } catch {}
          }, 10_000);
          guard.unref?.();
          tlsSock.once("close", () => clearTimeout(guard));
          try { tlsSock.end(Buffer.concat([head, body])); }
          catch { clearTimeout(guard); try { tlsSock.destroy(); } catch {} }
        };
