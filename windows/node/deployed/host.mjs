...
export class Host {
  ...
  async tick() {
    ...
    for (const id of [...this.tracked]) {
      ...
      // IS IT STILL ANSWERING? alive() existed and was asked only by proveAll, where a silent app
      // merely loses a checkpoint - so two apps sat at "running" for two hours while every
      // connection to their port was refused. This records what is true, and recovers from it.
      const liveApp = this.apps.get(id);
      if (liveApp && ["running", "unreachable"].includes(rec.status)) {
        const answering = await liveApp.alive().catch(() => false);
        if (!answering) {
      ...
      } else if (liveApp) { this.deadSince.delete(id); }
  ...
  /**
   * Does this box HOLD this deployment? The relay asks every live box `HEAD /x/<id>` and takes the
   * first answer that is not 404, so this is a FLEET ROUTING decision. It used to be
   * `records.has(id)`, which is true of everything this box ever considered - refusals included -
   * so this node claimed deployments it had declined by name and took another box's tenant.
   * "Here" is a lease this box tracks whose record is claiming, provisioning, running or
   * unreachable: the relay must not disown a box whose app is coming up or briefly silent.
   */
  holdsLease(id) {
    const key = String(id || "").toLowerCase();
    ...
    if (!rec) return false;
    return ["claiming", "provisioning", "running", "unreachable"].includes(String(rec.status || ""));
  }
