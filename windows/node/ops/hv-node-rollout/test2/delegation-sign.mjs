...
//   ETH_AGENT_WALLET=<key, in the environment only> node delegation-sign.mjs --operator 0x389C… --box nucbox-k11 \
//       (--days <n> | --minutes <n>) --expect-owner 0x29479bf0… --out <file.json> [--registry 0x…] [--chain 8453]
...
const v = await verifyDelegation(JSON.parse(fs.readFileSync(out, "utf8")), { operator, box, chain, registry });
...
console.log(`file: ${out}  sha256 ${(await import("node:crypto")).createHash("sha256").update(fs.readFileSync(out)).digest("hex")}`);
console.log(`verifyDelegation (the relay's and the node's module): ${v.ok ? `VALID, owner ${v.owner}` : `NOT VALID: ${v.reason}`}\n`);
