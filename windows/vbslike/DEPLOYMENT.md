...
- A trusted public certificate until B's hostname path is live on us-west (step 1b, HELD) AND the node attaches v2 (the v42
  node; the deployed `4ef0e862` attaches v1, host-only, so no certificate can issue for test 1 even with steps 1-3 on
  nan; enclave-d1).
...
| m3 monitor: user namespaces + io_uring | For the NEXT NucBox build, NOT the current candidate (enclave-87): the SNP guest's dominit now holds `user.max_user_namespaces=0` and `kernel.io_uring_disabled=2`, read back, fail closed (`isolation/snp-dominit-yama` `b63c2def`). The same two belong in m3's monitor beside its Yama hold (298924ae): IMPLEMENTED on `isolation/runtime-seccomp` `2ea664d8` (holdSysctl, read back, fail closed before `ready`), together with the runtime's seccomp filter (no AF_VSOCK, io_uring, namespaces, ptrace, ...; never the front) for the NucBox build after v42; in review (5d, then bf) | 5d |
