...
| M2 | **LANDED on main as `a60c415b`** (enclave-5d, pushed alone). The node's isolation code, the manager at `62965126` (ops `0513ced0`), and host/ at the shipped launcher's source `1a6f1556`. Deploy run 36138299668: only `detect` ran, with every output false, so NO release and no deploy. The patch-id is identical to 5ce3b6ed, approved by d1 (owner) and 99. Deploying a node to any box is separate and not requested. Next: the manager head `76af33b4` (v40's, box-verified 6/6) as its own exact revision `582e3019` (m2/manager-76af33b4), approved by d1 (owner); 99's approval pending; solo push after a fresh audit. | enclave-5d; landed |
