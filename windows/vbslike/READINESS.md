...
| U4 | Recovery after a HOST reboot (not a manager restart) | **VALIDATED, functional** (d1, `evidence/host-reboot-u4-20260925/`, harness `874169c5`+`5b14c067` reviewed by enclave-63). Two lab domains served as the host rebooted. After the boot: the VMs Off and not back by themselves; the manager recovered exactly those two, HELD (never served); the liveness sweep failed them as Off and left the VMs; the old routes and relay ports refuse; a fresh explicit launch served. Prior state restored (a one-shot SYSTEM task with literals); the legacy node task was disabled for that boot only and re-enabled byte-identical, not run. **Limits:** one graceful reboot (not power loss or a crash); the node's own retirement not exercised; functional only | done. The legacy node boot task `\EnclaveWindowsNode` has been DISABLED persistently since 15:56:32Z (per Steven's direction, at Codex's request; not deleted; rollback documented; `evidence/legacy-node-task-disabled-20260925/`) |
