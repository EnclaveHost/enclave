...
function host(over = {}) {
  ...
  const answers = {
    ...
    pinFirmware: { returnValue: 0, job: "" },
    attachConsole: { ok: true },
    start: { state: "Running" },
    teardown: { removed: 1, names: ["x"] },
    ...over,
  };
  const run = async (script) => {
    ...
    const key = script.includes("$r.vmms") ? "preflight"
      ...
      : script.includes("Remove-VM") ? "teardown"
...
test("teardown touches only what this instance owns - prefix AND marker, never one", () => {
  const s = CMD.teardown({ prefix: "enclave-app-t-" });
  assert.match(s, /\$_\.Name\.StartsWith\('enclave-app-t-'\) -and \$_\.Notes -eq/);
  assert.match(s, new RegExp(`-eq '${OWNER_MARKER}'`));
...
test("preflight reports each prerequisite, and start refuses before creating anything", async () => {
  ...
  await assert.rejects(() => l.start(mapping), /the Hyper-V role is not usable/);
...
test("a good start does the steps in order and hands back a stoppable handle", async () => {
  const h = host();
  const r = await mk(h).start(mapping);
  ...
  assert.deepEqual(order, ["preflight", "image", "create", "pin", "console", "start"],
                   "firmware is pinned BEFORE the VM starts, and the console before that too");
  assert.equal(r.state, "Running");
  assert.equal(r.name, "enclave-app-t-708e640945d1");
  assert.ok(r.pipe.startsWith("\\\\.\\pipe\\"), "a named pipe, so the guest can be heard at all");
  assert.equal(r.image.sha256, SHA);
});

test("ModifySystemSettings returning a job (4096) is success, anything else is not", async () => {
  assert.equal((await mk(host({ pinFirmware: { returnValue: 4096 } })).start(mapping)).state, "Running");
  await assert.rejects(() => mk(host({ pinFirmware: { returnValue: 32775 } })).start(mapping), /returned 32775/);
});

test("a VM older than the firmware field needs is refused", async () => {
  await assert.rejects(() => mk(host({ create: { id: "g", version: "9.0", name: "x" } })).start(mapping),
...
test("a failure after creation removes the half-built VM before rethrowing", async () => {
  for (const broken of [{ pinFirmware: new Error("wmi said no") }, { attachConsole: new Error("no pipe") }, { start: new Error("would not start") }]) {
    const h = host(broken);
    await assert.rejects(() => mk(h).start(mapping));
    assert.equal(h.seen.some((s) => s.includes("Remove-VM")), true,
