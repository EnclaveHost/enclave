...
# (C:\Users\claude\uefi-probe.lock), whose harness applies and restores the same settings.
...
function Assert-Writable {
  if (Test-Path $Lock) { Refuse "an acceptance run holds $Lock (its harness applies and restores the same settings); run this after it ends" }
  if (-not $Rehearsal) {
